...
namespace rs_disk_oriented {

// Allows building a `RadixSpline` in a single pass over sorted data.
template <class KeyType>
class Builder {
 public:
  Builder(KeyType min_key, KeyType max_key, size_t max_y,
          size_t record_per_page, size_t num_radix_bits = 18,
          size_t max_error = 32)
      : min_key_(min_key),
        max_key_(max_key),
        max_y_(max_y),
  ...
  // Adds a key with a given position. Modified for on-disk mode.
  ...
  rs::RadixSpline<KeyType> Finalize() {
    ...
    return rs::RadixSpline<KeyType>(min_key_, max_key_, max_y_, curr_num_keys_,
                                    num_radix_bits_, num_shift_bits_,
                                    max_error_, std::move(radix_table_),
 ...
 private:
  ...
  // Disk-oriented Algorithm
  static std::pair<double, double> GetRangeY(size_t y, size_t eps,
    ...
    if (!cross_page && eps < record_per_page / 4) {
      // excessive merging -> avg page becomes large
      y_low = zero_left + eps;
      y_high = zero_right - eps;
    }
  ...
  void PossiblyAddKeyToSpline(KeyType key, double position) {
    ...
    // Modified for on-disk mode
    if (key == prev_key_ && position == prev_position_) {
    ...
    size_t cross_num = (2 * max_error_) * 1.0 / record_per_page_ * (max_y_ + 1);
    size_t cross_gap =
        (max_y_ + 1) / 2 > cross_num ? (max_y_ + 1) / 2 / cross_num : 1.0;
    bool cross_page = false;
    if (static_cast<uint64_t>(position) % cross_gap == 0) {
      cross_page = true;
    }

    auto y_range =
        GetRangeY(position, max_error_, record_per_page_, cross_page);
    const double upper_y = y_range.second;
    const double lower_y = y_range.first;
    // const double upper_y = position + max_error_;
    // const double lower_y = (position < max_error_) ? 0 : position -
    // max_error_;
    ...
    // `B` in algorithm.
    const rs::Coord<KeyType>& last = spline_points_.back();
    ...
    assert(prev_point_.x != last.x || prev_point_.y != last.y);
  ...
  rs::Coord<KeyType> upper_limit_;
  rs::Coord<KeyType> lower_limit_;

  // Previous CDF point.
  rs::Coord<KeyType> prev_point_;
};
