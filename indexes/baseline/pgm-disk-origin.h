#ifndef INDEXES_BASELINE_PGM_DISK_ORIGIN_H_
...
class BaselinePGMDiskOrigin : public BaseIndex<K, V> {
 public:
  struct param_t {
    size_t eps;
  ...
  typedef pgm_baseline_disk::DynamicPGMIndexOrigin<K, V> PGM_DISK;

  BaselinePGMDiskOrigin(param_t params = param_t(64, ""))
  ...
  void Build(typename BaseIndex<K, V>::DataVec_ &key_value) {
    ...
    pgm_.build(const_cast<char *>(index_file_.c_str()), true, pgm_inner_on_disk,
               key_value.begin(), key_value.end(), eps_);
  ...
  std::string GetIndexName() const { return name_ + std::to_string(eps_); }

  param_t GetIndexParams() const { return param_t(0); }

 private:
  std::string name_ = "BASELINE_PGM_DISK_ORIGIN";
...
#endif  // !INDEXES_BASELINE_PGM_DISK_ORIGIN_H_