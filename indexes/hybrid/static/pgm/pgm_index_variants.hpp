...
public:
    ...
    CompressedPGMIndex(Iterator first, Iterator last, size_t epsilon) : n(std::distance(first, last)) {
        ...
        auto ignore_last = (*std::prev(last)).first == std::numeric_limits<K>::max(); // max is reserved for padding
        ...
        auto in_fun = [&](auto i) {
            ...
            auto flag = i > 0 && i + 1u < n && x == first[i - 1].first && x != first[i + 1].first && x + 1 != first[i + 1].first;
        ...
        for (int i = EpsilonRecursive == 0 ? 1 : n_levels - 1; i > 0; --i) {
            ...
            levels.emplace_back(segments.begin() + l, segments.begin() + r,
                                ...
                                slopes_table, prev_level_size, (*std::prev(last)).first);
        }
    }
