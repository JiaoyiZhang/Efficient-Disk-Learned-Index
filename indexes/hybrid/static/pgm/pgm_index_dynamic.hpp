...
class DynamicPGMIndex {
  ...
  const uint8_t
      min_index_level;     ///< Minimum level on which an index is constructed.
  ...
  void insert(const Item &new_item) {
    ...
    if (need_new_level) {
      ++used_levels;
      levels.emplace_back();
      if (i - min_index_level >= int(pgms.size())) pgms.emplace_back();
    }
 ...
 public:
   ...
   * speed up searches
   */
  DynamicPGMIndex(uint8_t base = 8, uint8_t buffer_level = 0,
      ...
      : base(base),
        ...
        min_index_level(std::max<size_t>(
            min_level + 1,
            index_level ? index_level : ceil_log_base(size_t(1) << 16))),
        buffer_max_size(),
  ...
  DynamicPGMIndex(Iterator first, Iterator last, uint8_t base = 8,
  ...
  template <typename Iterator>
  void bulk_load(Iterator first, Iterator last, uint8_t base = 8,
                 uint8_t buffer_level = 0, uint8_t index_level = 0) {
    DynamicPGMIndex(base, buffer_level, index_level);
    size_t n = std::distance(first, last);
    used_levels = std::max<uint8_t>(ceil_log_base(n), min_level) + 1;
