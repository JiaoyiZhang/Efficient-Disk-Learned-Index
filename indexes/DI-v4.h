#ifndef INDEXES_DI_V4_H_
#define INDEXES_DI_V4_H_
...
class DI_V4 : public BaseIndex<K, V> {
 public:
  struct param_t {
    float lambda;
  ...
  DI_V4(param_t p = param_t(1.25, 256)) {
    lambda_ = p.lambda;
  ...
  std::string GetIndexName() const override {
    ...
    return "DI-V4_" + str0.substr(0, str0.find(".") + 3);
  }
