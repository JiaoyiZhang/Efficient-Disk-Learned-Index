...
class OptimalPiecewiseLinearModel {
 ...
 public:
  ...
  bool add_point(const X &x, const Y &y_low, const Y &y_high, const Y &now_y) {
    ...
    auto min_y = std::numeric_limits<Y>::lowest();
    //==============================================================
    //  Use zero-error range to accommodate more points in a model
    //==============================================================
    Point p1{x, y_high >= max_y - epsilon ? max_y : y_high};
    Point p2{x, y_low <= min_y + epsilon ? min_y : y_low};
 ...
 */
template <typename Fin, typename Fout>
size_t make_segmentation_range(size_t n, size_t epsilon, Fin in, Fout out) {
  if (n == 0) return 0;
...
size_t make_segmentation_range_par(size_t n, size_t epsilon, Fin in, Fout out) {
  ...
  if (parallelism == 1 || n < 1ull << 15)
    return make_segmentation_range(n, epsilon, in, out);
...
#pragma omp parallel for reduction(+ : c) num_threads(parallelism)
  for (auto i = 0; i < parallelism; ++i) {
    ...
    c += make_segmentation_range(last - first, epsilon, in_fun, out_fun);
  }
