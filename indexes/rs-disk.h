#ifndef INDEXES_RS_DISK_H_
#define INDEXES_RS_DISK_H_
...
class RSIndex : public BaseIndex<K, V> {
 public:
  struct param_t {
    size_t num_radix_bits;
    size_t max_error;
  };
  RSIndex(param_t p = param_t(18, 1)) {
  ...
  std::string GetIndexName() const override {
    return "RadixSplineDisk-" + std::to_string(num_radix_bits_) + "_" +
