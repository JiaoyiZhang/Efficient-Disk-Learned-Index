...
typedef uint64_t key_type;  // if (filename== "books" || filename == "wiki_ts",
                            // "synthetic", "YCSB")

// typedef long int key_type;  // if (filename== "books" || filename ==
// "wiki_ts",
//                             // "synthetic", "YCSB")
...
#define PGM_SUB_EPS2(x, epsilon, size) \
  ((x) <= (epsilon) || (x) - (epsilon) >= (size) ? 0 : ((x) - (epsilon)))
#define PGM_ADD_EPS(x, epsilon, size) \
  ((x) + (epsilon) + 2 >= (size) ? (size) : (x) + (epsilon) + 2)

// record the access_stats, the number of access memory, the number of access
// disk and the number of cross memory and disk;
struct access_stats {
  int memnum = 0;
  int disknum = 0;
  int crossnum = 0;
  ...
  void print_stats() {
    ...
    cout << "ID " << double(t1.tv_sec) << " qtype " << qtype << " zipffactor "
         << zipffactor << " insert_frac " << insert_frac
         << " out_of_order_frac " << out_of_order_frac << " memnum " << memnum
         << " disknum " << disknum << " crossnum " << crossnum
         << " diskpagenum ";
    cout << diskpagenum << " crosspagenum " << crosspagenum << " pagecross "
         << pagecross << " writenum " << writenum << " querytimeuse "
         << querytimeuse << " lrutimeuse " << lrutime << " disktimeuse "
         << disktime << " readdisktimeuse " << rdisktime << " writedisktimeuse "
         << wdisktime << " indexloopuptimeuse " << xlookuptime
         << " sort_piecetime " << sort_piecetime << " workload " << workload
         << "\n";
    // ofstream savefile;
    // savefile.open(
    //     "/home/wamdm/chaohong/clionDir/updatefilm/result/"
    //     "adalru_film_performance.txt",
    //     ios::app);
    // //            savefile << "_ ";
    // savefile << "ID " << double(t1.tv_sec) << " qtype " << qtype
    //          << " zipffactor " << zipffactor << " insert_frac " << insert_frac
    //          << " out_of_order_frac " << out_of_order_frac << " memnum "
    //          << memnum << " disknum " << disknum << " crossnum " << crossnum
    //          << " diskpagenum ";
    // savefile << diskpagenum << " crosspagenum " << crosspagenum << " pagecross "
    //          << pagecross << " writenum " << writenum << " querytimeuse "
    //          << querytimeuse << " lrutimeuse " << lrutime << " disktimeuse "
    //          << disktime << " readdisktimeuse " << rdisktime
    //          << " writedisktimeuse " << wdisktime << " indexloopuptimeuse "
    //          << xlookuptime << " sort_piecetime " << sort_piecetime
    //          << " workload " << workload << "\n";
    // savefile << flush;
    // savefile.close();
  }
...
struct film_stats {
  ...
  /// Zero initialized
  inline film_stats() : leaves(0), numlevel(0) {}
};
...
class FILMinsert {
 public:
  ...
  struct sortPiece : Leafpiece {
    ...
    //        vector< pair<bool, adalru::Node<key_type, std::vector<key_type> >*
    //        > >slotdata;
    std::vector<bool> slotflag;  // adalru::Node<key_type, std::vector<key_type>
                                 // >*sss
    std::vector<void *>
        slotdata;  // adalru::Node<key_type, std::vector<key_type> >*sss
    adalru::localLRU<lruOff_type, key_type *> intrachain;  // unsigned short int

    sortPiece() = default;

    friend inline bool operator<(const sortPiece &p, const key_type &key) {
      return p.key < key;
    }
    friend inline bool operator<(const key_type &key, const sortPiece &p) {
      return key < p.key;
    }
    friend inline bool operator<(const sortPiece &p1, const sortPiece &p2) {
    ...
    inline int operator()(const key_type key) const {
      int begin = 0, end = slotkey.size() - 1,
          mid;  // find last Equal or small, 找到最后一个大于或等于 key 的
      while (begin <= end) {
        mid = (end + begin) / 2;
        if (slotkey[mid] >= key) {
          end = mid - 1;
    ...
    inline bool insert(key_type key, key_type *payload) {
      int begin = 0, end = slotkey.size() - 1,
          mid;  // find last Equal or small, 找到最后一个大于或等于 key 的
      while (begin <= end) {
        mid = (end + begin) / 2;
      ...
      this->slotdata.insert(slotdata.begin() + end + 1,
                            intrachain.put(end + 1, payload));
    ...
    // 判断key 是否在当前leaf 中
    inline pair<bool, unsigned long> is_in_leaf(const key_type &key,
                                                const int error) {
  ...
  struct Innerlevel;  // 声明 innerlevel 结构体，每个innerllevel 包含一个
                      // vector<Innerpiece>,一个 opt， 一个 pos；
  struct Leaflevel;
  ...
  //        typedef std::pair<
  //        std::vector<Leafpiece*>,vector<internal::insertPWLF<key_type, int>*>
  //        > leafleveltype;
  Leaflevel leaflevel;  //  the leafpieces composing the leaflevel
                        //        typedef std::pair< std::vector<Innerpiece>,
  //        vector<internal::insertPWLF<key_type, int>*> > innerleveltype;
   ...
   * @param keys the vector of keys to be indexed (sorted). @param payload the
   * value of each key
   */
  FILMinsert() {}
  ...
  void delete_leaf(Leafpiece *node) {
    if (node == NULL) {
      ...
      //                }
      //                adalru::localLRU <lruOff_type,key_type* >  intrachain;
      //                //unsigned short int
      delete node;
      node = NULL;
  ...
  void release() {
    ...
    vector<FILMinsert<key_type, value_type>::Leafpiece *>().swap(
        leaflevel.leafpieces);
    delete leaflevel.opt;
    for (unsigned int leveli = 0; leveli < innerlevels.size(); leveli++) {
      auto level_it = innerlevels[leveli];
      //                for (unsigned int inneri = 0; inneri <
      //                level_it->innerpieces.size();inneri ++ ){
  ...
  inline std::vector<key_type> append_one(size_t error,
                                          std::vector<key_type> keys,
                                          unsigned int k) {
    std::vector<key_type> startkeys;

    for (size_t i = 0; i < keys.size(); ++i) {
      pair<key_type, unsigned int> p(keys[i],
                                     innerlevels[k]->nextpos++);  // i 为 pos
      ++(innerlevels[k]->pos);
      if (!innerlevels[k]->opt->add_point(
              p.first, p.second)) {  // 如果inner level  不满足error
                                     // 了，那么再创建一个innerpiece
        // 当前 innerpiece 不再满足，需要创建new inner piece 并判断该 innerlevel
        // 的上一层level 是否需要更新
        auto a = innerlevels[k]->opt->get_segment();

        if (innerlevels[k]->pos > 2) innerlevels[k]->innerpieces.pop_back();
        innerlevels[k]->innerpieces.emplace_back(a);
        //                    auto newinner =
        //                    innerlevels[k]->innerpieces.back(); cout<< "i need
        //                    You, my lovely Lord, why there is nan" << endl;

        // 首先在该层创建一个 new innerpiece， 更新该innerpiece，再递归向向上
        //                    typename filmtype::Innerpiece *innerpiece = new
        //                    typename filmtype::;
        innerlevels[k]->pos = 0;
        innerlevels[k]->nextpos -= 2;
        delete innerlevels[k]->opt;
        //                    innerlevels[k]->opt.pop_back();
        internal::insertPWLF<key_type, int> *inneropt =
            new internal::insertPWLF<key_type, int>(error);
        innerlevels[k]->opt = inneropt;
        if (k == 0) {
          //                        auto aaaaa = leaflevel.leafpieces.size()-2;
          startkeys.emplace_back(
              leaflevel.leafpieces[leaflevel.leafpieces.size() - 1]->startkey);
        }

        else {
          //                        auto aaaaa =
          //                        innerlevels[k-1]->innerpieces.size()-2;
          startkeys.emplace_back(
              innerlevels[k - 1]
                  ->innerpieces[innerlevels[k - 1]->innerpieces.size() - 2]
                  .startkey);
        }
        ...
        if (innerlevels.back()->innerpieces.size() > 1) {
          ...
          internal::insertPWLF<key_type, int> *inneropt =
              new internal::insertPWLF<key_type, int>(error);
          //                    std::pair< std::vector<typename filmtype::
          //                    Innerpiece>,
          //                    std::vector<internal::insertPWLF<key_type,
          //                    int>*> > *innerlevel =
          //                            new std::pair< std::vector<typename
          //                            filmtype:: Innerpiece>,
          //                            std::vector<internal::insertPWLF<key_type,
          //                            int>*> >;
          ...
          return startkeys;
        } else if (innerlevels.size() > 1 &&
                   (k !=
                    innerlevels.size() -
                        1))  // 如上为 由于创建了new innner piece，导致了new
                             // innerlevel，如下为，虽然创建了new
                             // innerpiece，但只需要更新上层的innner level
        {
  ...
  inline void append_one(key_type key, key_type *payload, unsigned int error,
                         lru_type interchain) {
    std::vector<key_type> startkeys;
    inkeynum++;
    ...
    if (leaflevel.opt->append_point(p.first, p.second)) {
      ...
      m_tailleaf->slotdata.emplace_back(
          m_tailleaf->intrachain.put(p.second, payload));
      m_tailleaf->slotflag.emplace_back(true);
    } else {
      auto a = leaflevel.opt->get_segment(
          m_tailleaf->endkey);  // 将生成的new leaf piece插入到leaflevel 中
      m_tailleaf->update(a);
      ...
      if (innerlevels.size() == 0) {
        ...
        //                    std::pair< std::vector<typename filmtype::
        //                    Innerpiece>,
        //                    std::vector<internal::insertPWLF<key_type, int>*>
        //                    > *innerlevel =
        //                            new std::pair< std::vector<typename
        //                            filmtype:: Innerpiece>,
        //                            std::vector<internal::insertPWLF<key_type,
        ...
        startkeys.emplace_back(p.first);  // p.first is the break_key, and also
                                          // the startkey of the next leaf piece
  ...
  template <class lru_type>  // insert_bulk_load_one_by_one (also one by one),
                             // in the beginning, the index must be empty? maybe
                             // not  empty is okay
  inline void update_append(std::vector<key_type> keys, key_type *payload,
                            unsigned int error, unsigned int error_recursize,
                            lru_type &interchain) {
    // build leaf level
    ...
    std::pair<size_t, std::vector<key_type>> n_parts =
        internal::append_segmentation(error, keys, payload, this, inkeynum,
                                      this->m_tailleaf,
                                      interchain);  //<key_type,Leafpiece>
    //            std::cout<< "You are my refuge, from ever to ever"<<endl;
    // on the basis of leaf level, build the upper level until just has one
    // piece, that the make_segmentation just return 1.
    if (innerlevels.size() > 1) root = &innerlevels.back()->innerpieces[0];
    this->verify_film(&vstats);
    cout << "index append test finished,Jesus, my Lord, i need You, praise to "
            "You for ever and ever!"
         << endl;
  }

  /*
  inline void update_random(std::vector<key_type> keys,std::vector<key_type>
  payload){
  ...
  */
  ...
  inline void update_random(std::vector<key_type> keys, key_type *payload,
                            lru_type *interchain) {
    // 首先search , 如果不存在，则插入，如果存在，则 pass
    unsigned int i = 0;
    if (sort_list == NULL) {
      ...
      this->sort_list->slotdata.emplace_back(
          sort_list->intrachain.put(0, payload));
    }
    for (; i < keys.size(); i++) {
  ...
  template <class lru_type>
  inline void update_random(const key_type key, key_type *payload,
                            lru_type *interchain) {
    // 首先search , 如果不存在，则插入，如果存在，则 pass
    if (sort_list == NULL) {
      sort_list = new sortPiece();
      this->sort_list->slotkey.emplace_back(key);
      this->sort_list->slotflag.emplace_back(true);
      this->sort_list->slotdata.emplace_back(
          sort_list->intrachain.put(0, payload));
    }
    this->update_one(key, payload, interchain);
  ...
  struct range_res_find {
    ...
    inline range_res_find(bool f, key_type firstkey, key_type lastkey)
        : flags(f),
          firstkey(firstkey),
          lastkey(),
          findleafs(),
    ...
    inline range_res_find(bool f, key_type firstkey, int first_loc,
                          key_type lastkey)
        : flags(f),
          firstkey(firstkey),
          findleafs(),
    ...
    inline bool in_or_out() { return true; }
  };

  /// film recursive find the key has much information which is needs to be
  /// return.
  struct result_find {
    /// find result flags
    ...
    /// Constructor with a lastkey value.
    inline result_find(bool find_f, bool in_or_exf, Leafpiece *find_leaf,
                       int loc_slot)
        : find(find_f), flags(in_or_exf), findleaf(find_leaf), slot(loc_slot) {}

    /// Test if this result object has a given flag set.
    bool has(bool f) const { return (flags & f) != 0; }
  ...
  /** search method
   * the methods used in query, @param key is the queried key, return the
   * leafpiece responsible for the given key
   */

  inline result_find search_one(const key_type key,
                                access_stats *query_stats) const {
    ...
    if (vstats.innernum > 1) {
      ...
      auto pos = (*root)(key);
      auto lo = PGM_SUB_EPS(pos, ErrorRecursive + 1) +
                (*(--itlevel))->innerpieces.begin();  //,(--itlevel)->size()
      auto hi =
          PGM_ADD_EPS(pos, ErrorRecursive, (*itlevel)->innerpieces.size() - 1) +
          (*itlevel)->innerpieces.begin();
      auto curit = (*(itlevel--))->innerpieces.begin();
      // 在 low 和 high 范围内的cand;idate innerpieces  中 确定所属的 inner
      // piece
      static constexpr size_t linear_search_threshold =
          8 * 64 / sizeof(Innerpiece);  // sizeof(Innerpiece)
      if (key >= hi->startkey) {
        curit = hi;
      } else {
        if constexpr (4 <= linear_search_threshold) {  // ErrorRecursive
          for (; std::next(lo)->startkey <= key; ++lo) continue;
          curit = lo;
        } else {
          curit = std::lower_bound(
              lo, hi,
              key);  // upper_bound 返回在前闭后开区间查找key 的上届，返回大于
                     // key 的第一个元素位置； lower_bound 返回第一个大于或等于
                     // key 的元素的位置
        }
      }
      ...
      for (; itlevel >= innerlevels.begin(); --itlevel) {
        pos = (*curit)(key);
        lo = PGM_SUB_EPS(pos, ErrorRecursive + 1) +
             (*itlevel)->innerpieces.begin();
        hi = PGM_ADD_EPS(pos, ErrorRecursive,
                         (*itlevel)->innerpieces.size() - 1) +
             (*itlevel)->innerpieces.begin();
        // 在 low 和 high 范围内的candidate innerpieces  中 确定所属的 inner
        // piece
        static constexpr size_t linear_search_threshold =
            8 * 64 / sizeof(Innerpiece);  // sizeof(Innerpiece)
        if (key >= hi->startkey) {
          ...
          if constexpr (4 <= linear_search_threshold) {  // ErrorRecursive
            ...
            curit = std::lower_bound(
                lo, hi,
                key);  // upper_bound 返回在前闭后开区间查找key 的上届，返回大于
                       // key 的第一个元素位置； lower_bound
                       // 返回第一个大于或等于 key 的元素的位置
          }
        }
      }
      ...
      auto leaflo = PGM_SUB_EPS2(pos, Error + 1, leaflevel.leafpieces.size()) +
                    leaflevel.leafpieces.begin();
      auto leafhi = PGM_ADD_EPS(pos, Error, leaflevel.leafpieces.size() - 1) +
                    leaflevel.leafpieces.begin();

      Leafpiece *x = *leaflo;
      Leafpiece *y = *leafhi;
      if (key >= (*leafhi)->startkey) {
        curleaf = leafhi;
      } else {
        if constexpr (4 <= linear_search_threshold) {  // ErrorRecursive
          for (; (*leaflo)->endkey < key; leaflo++) continue;
          curleaf = leaflo;
        } else {
          curleaf = std::lower_bound(
              leaflo, leafhi,
              key);  // upper_bound 返回在前闭后开区间查找key 的上届，返回大于
                     // key 的第一个元素位置； lower_bound 返回第一个大于或等于
                     // key 的元素的位置
      ...
      if (key != a->slotkey[resslot]) {
        //                    if (sort_list==NULL) {
        //                        cout << " i need You, my Lord, please help
        //                        me!" << endl;
        //                    }
        // find in sort_list
        ...
        if (sort_list->slotkey[sortpos] == key) {
          //                            auto sss1 =
          //                            sort_list->slotkey[sortpos+1]; auto sss2
          //                            = sort_list->slotkey[sortpos]; auto sss3
          //                            = sort_list->slotkey[sortpos-1]; auto
          //                            sort_len = sort_list->slotkey.size();
          result_find index_res =
              result_find(false, sort_list->slotflag[sortpos], sort_list,
                          sortpos);  // sort_list data
          return index_res;
        } else {
          cout << " i need You, my Lord, please help me! not find the key"
               << endl;
        }
      }
      gettimeofday(&ct2, NULL);
      ctimeuse = (ct2.tv_sec - ct1.tv_sec) +
                 (double)(ct2.tv_usec - ct1.tv_usec) / 1000000.0;
      query_stats->sort_piecetime += ctimeuse;
      result_find index_res =
          result_find(true, a->slotflag[resslot], a, resslot);  // regular data
      return index_res;
    } else {
      // 只有一层leaf
      static constexpr size_t linear_search_threshold =
  ...
  inline range_res_find search_range(const key_type firstkey,
                                     const key_type lastkey,
                                     access_stats *query_stats) const {
    struct timeval ct1, ct2;
    double ctimeuse;
    if (vstats.innernum > 1) {
      ...
      static constexpr size_t linear_search_threshold =
          8 * 64 / sizeof(Innerpiece);  // sizeof(Innerpiece)
      if (firstkey >= hi->startkey) {
        curit = hi;
      } else {
        if constexpr (4 <= linear_search_threshold) {  // ErrorRecursive
          for (; std::next(lo)->startkey <= firstkey; ++lo) continue;
          curit = lo;
        } else {
          curit = std::lower_bound(
              lo, hi,
              firstkey);  // upper_bound 返回在前闭后开区间查找key
                          // 的上届，返回大于 key 的第一个元素位置； lower_bound
                          // 返回第一个大于或等于 key 的元素的位置
        }
      }
      // 在lo 和 hi 的范围内，定位 key 所属的inner piec, 最后定位到leaf piece
        ...
        if (firstkey >= hi->startkey) {
          ...
          if constexpr (4 <= linear_search_threshold) {  // ErrorRecursive
            ...
            curit = std::lower_bound(
                lo, hi,
                firstkey);  // upper_bound 返回在前闭后开区间查找key
                            // 的上届，返回大于 key 的第一个元素位置；
                            // lower_bound 返回第一个大于或等于 key 的元素的位置
          }
        }
      }
      ...
      //                Leafpiece* x = *leaflo;
      //                Leafpiece* y = *leafhi;
      if (firstkey >= (*leafhi)->startkey) {
        curleaf = leafhi;
      } else {
        if constexpr (4 <= linear_search_threshold) {  // ErrorRecursive
          for (; (*leaflo)->endkey < firstkey; leaflo++) continue;
          curleaf = leaflo;
        } else {
          curleaf = std::lower_bound(
              leaflo, leafhi,
      ...
      if (firstkey != a->slotkey[resslot])
      {
          ...
          if (sort_list->slotkey[sortpos] == firstkey){
              result_find index_res =
      result_find(false,sort_list->slotflag[sortpos],sort_list,sortpos);   //
      sort_list data return index_res;
          }
          else{
              cout << " i need You, my Lord, please help me! not find the key"
      << endl;
      ...
      gettimeofday(&ct2, NULL);
      ctimeuse = (ct2.tv_sec - ct1.tv_sec) + (double) (ct2.tv_usec -
      ct1.tv_usec) / 1000000.0; query_stats->sort_piecetime += ctimeuse;
      */

      range_res_find rangeresult =
          range_res_find(true, firstkey, resslot, lastkey);
      ...
      {
//                    if (sort_list==NULL) {
//                        cout << " i need You, my Lord, please help me!" <<
endl;
//                    }
//                    else {  // find in sort_list
          auto sortpos = (*sort_list)(firstkey);
...
ct1.tv_usec) / 1000000.0; query_stats->sort_piecetime += ctimeuse;
  ...
  template <class lru_type>
  inline void update_one(const key_type key, key_type *payload,
                         lru_type interchain) {
  ...
  /*
          bool update_search_one(const key_type key, const vector<key_type>
  payload) const {
              // 从 root 开始，即，从root开始的结尾开始
              if (vstats.innernum > 1){
                  auto itlevel = innerlevels.end() -1; // root level 的 ittrater
                  auto pos = (*root)(key);
                  auto lo = PGM_SUB_EPS(pos, ErrorRecursive + 1) +
  (*(--itlevel))->innerpieces.begin();  //,(--itlevel)->size() auto hi =
  PGM_ADD_EPS(pos, ErrorRecursive,(*itlevel)->innerpieces.size()-1) +
  (*itlevel)->innerpieces.begin(); auto curit =
  (*(itlevel--))->innerpieces.begin();
                  // 在 low 和 high 范围内的cand;idate innerpieces  中
  确定所属的 inner piece static constexpr size_t linear_search_threshold = 8 *
  64 / sizeof(Innerpiece);   //sizeof(Innerpiece) if (key >= hi->startkey){curit
  = hi;} else{ if constexpr (4 <= linear_search_threshold ){   //ErrorRecursive
                          for (; std::next(lo)->startkey <= key; ++lo)
                              continue;
                          curit = lo;
                      }
                      else{ curit = std::lower_bound(lo, hi, key);   //
  upper_bound 返回在前闭后开区间查找key 的上届，返回大于 key 的第一个元素位置；
  lower_bound 返回第一个大于或等于 key 的元素的位置
                      }
                  }
                  // 在lo 和 hi 的范围内，定位 key 所属的inner piec,
  最后定位到leaf piece for (; itlevel >= innerlevels.begin();--itlevel  ){ pos =
  (*curit)(key); lo = PGM_SUB_EPS(pos, ErrorRecursive + 1) +
  (*itlevel)->innerpieces.begin(); hi = PGM_ADD_EPS(pos,
  ErrorRecursive,(*itlevel)->innerpieces.size()-1) +
  (*itlevel)->innerpieces.begin();
                      // 在 low 和 high 范围内的candidate innerpieces  中
  确定所属的 inner piece static constexpr size_t linear_search_threshold = 8 *
  64 / sizeof(Innerpiece);   //sizeof(Innerpiece) if (key >= hi->startkey){curit
  = hi;} else{ if (key <= (*lo).startkey){ pos = std::min<size_t>((*curit)(key),
  (*curit).intercept); lo = PGM_SUB_EPS(pos, ErrorRecursive + 1) +
  (*itlevel)->innerpieces.begin(); hi = PGM_ADD_EPS(pos,
  ErrorRecursive,(*itlevel)->innerpieces.size()-1) +
  (*itlevel)->innerpieces.begin();
                          }
                          if constexpr (4 <= linear_search_threshold ){
  //ErrorRecursive for (; std::next(lo)->startkey <= key; ++lo) continue; curit
  = lo;
                          }
                          else{ curit = std::lower_bound(lo, hi, key);   //
  upper_bound 返回在前闭后开区间查找key 的上届，返回大于 key 的第一个元素位置；
  lower_bound 返回第一个大于或等于 key 的元素的位置
                  ...
                  auto curleaf = leaflevel.leafpieces.begin();
                  pos = (*curit)(key);
  //                auto leaflo = PGM_SUB_EPS(pos, Error + 2) +
  leaflevel.begin(); auto leaflo = PGM_SUB_EPS2(pos, Error+1
  ,leaflevel.leafpieces.size()) + leaflevel.leafpieces.begin(); auto leafhi =
  PGM_ADD_EPS(pos, Error,leaflevel.leafpieces.size()-1) +
  leaflevel.leafpieces.begin();
  //                if (key <= (*leaflo)->startkey) {
  //                    pos = std::min<size_t>((*curit)(key),
  (*curit).intercept);
  //                    leaflo = PGM_SUB_EPS(pos, Error + 1) +
  leaflevel.leafpieces.begin();
  //                    leafhi = PGM_ADD_EPS(pos, Error,
  leaflevel.leafpieces.size() - 1) +  leaflevel.leafpieces.begin();
  //                }
                  ...
                  if (key >= (*leafhi)->startkey){curleaf = leafhi;}
                      ...
                      if constexpr (4 <= linear_search_threshold ){
  //ErrorRecursive for (; (*leaflo)->endkey < key; leaflo++) continue; curleaf =
  leaflo;
                      }
  ...
  lower_bound 返回第一个大于或等于 key 的元素的位置
                  ...
                  int slotlo = PGM_SUB_EPS(pos, ErrorRecursive + 1) ;
  //,(--itlevel)->size() int slothi = PGM_ADD_EPS(pos,
  ErrorRecursive,(a->slotkey.size()-1)); int resslot = slotlo; for (;
  resslot<slothi;resslot++){ if (key >= a->slotkey[resslot])
                      {
                          break;
                      }
                  }
  ...
  endl; return true;
                          ...
                          int begin = 0, end = a->sort_list.size()-1,mid;  //
  find last Equal or small, 找到最后一个大于或等于 key 的 while(begin <= end){
                              mid = (end + begin) / 2;
                          ...
                          if(a->sort_list[end+1].first!=key){
                              a->sort_list.insert(a->sort_list.begin()+begin,pair<key_type,vector<key_type>
  >(key,payload)); return true;
                          }
                          return false;
                      }
                  }
                  ...
                  // 只有一层leaf
                  static constexpr size_t linear_search_threshold = 8 * 64 /
  sizeof(Innerpiece);   //sizeof(Innerpiece)
  //                if (key == 80728301)
  ...
  //                auto l = PGM_SUB_EPS2(pos, Error + 1,leaflevel.size());
                  auto leaflo = PGM_SUB_EPS2(pos, Error +
  1,leaflevel.leafpieces.size()) + leaflevel.leafpieces.begin(); auto leafhi =
  PGM_ADD_EPS(pos, Error,leaflevel.leafpieces.size()-1) +
  leaflevel.leafpieces.begin(); auto h = PGM_ADD_EPS(pos,
  Error,leaflevel.leafpieces.size()-1);
  //
  //                Leafpiece*  y= *leafhi;
  //                Leafpiece*  z= *leaflo;
                  if (key >= (*leafhi)->startkey){curleaf = leafhi;}
                      ...
                      if constexpr (4 <= linear_search_threshold ){
  //ErrorRecursive for (; (*leaflo)->endkey < key; leaflo++){
  //                            Leafpiece*  x= *leaflo;
                              continue;
                          }
  ...
  */
  ...
  size_t height() const { return innerlevels.size() + 1; }
  ...
  const map<string, double> runtimeshow_verify() {
    ...
    double leafnodesize =
        32;  // startkey, endkey, double(slope,intercept),  slotkey(算入了data
             // usage 中，因为在introchain 中 只存储了value)
    ...
    double addusage =
        double((exkeynum + inkeynum) * (1 + 8) +
               exkeynum * (sizeof(pageid_type) + sizeof(pageOff_type)) +
               exkeynum * sizeof(key_type)) /
        1048576;  //
    // addusage: (exkeynum+inkeynum)*(1+8) —— bitmap + pointer;  exkeynum*8 ——
    // pageid(ungined int),offset(unsigned int); exkeynum*sizeof(key_type)
    // ——exkey;
    double indexusge = double(vstats.leaves * leafnodesize +
                              vstats.innernum * innodesize + 8) /
                       1048576;  //  leaf nodes, inner nodes, root
    double lruusage =
        double(no_hashsize * inkeynum + hashlrusize * vstats.leaves) / 1048576;
    ...
    treeinfo.insert(pair<std::string, double>(
        "addusage", addusage));  // key,flag,pageID,offset
  ...
  const map<string, double> show_verify() {
    ...
    this->verify_film(&vstats);
    double innodesize =
        sizeof(key_type) + 8 + 8;  // startkey, double(slope,intercept)
    double leafnodesize =
        2 * sizeof(key_type) + 8 +
        8;  // startkey, endkey, double(slope,intercept),  slotkey(算入了data
            // usage 中，因为在introchain 中 只存储了value)
    ...
    double addusage =
        double((exkeynum + inkeynum) * (1 + 8) +
               exkeynum * (sizeof(pageid_type) + sizeof(pageOff_type)) +
               exkeynum * sizeof(key_type)) /
        double(1048576);  // exkey, flag, pageID,offset   (pageid-int-8 offset
                          // short int), flag for data in memory
    // addusage: (exkeynum+inkeynum)*(1+8) —— bitmap + pointer;  exkeynum*8 ——
    // pageid(ungined int),offset(unsigned int); exkeynum*sizeof(key_type)
    // ——exkey;
    double indexusge =
        (vstats.leaves * leafnodesize + vstats.innernum * innodesize + 8) /
        double(1048576);  //  leaf nodes, inner nodes, root
    double lruusage =
        double(no_hashsize * inkeynum + hashlrusize * vstats.leaves) / 1024 /
        1024;
  ...
  vector<int> traverse_leaves() {  // traverse the leaf level, find the min leaf
                                   // and the max leaf
    leaflevel;
    ...
    int lastslot =
        leaflevel.leafpieces[leaflevel.leafpieces.size() - 1]->slotkey.size();
...
struct FILMinsert<key_type, value_type>::Leafpiece {
  ...
  adalru::localLRU<lruOff_type, key_type *> intrachain;  // unsigned short int
  //        vector< pair<key_type,vector<key_type>> > sort_list;    // 用于 存储
  //        任意位置插入的数据
  double slope;
  double intercept;
  ...
  inline explicit Leafpiece(
      const typename internal::insertPWLF<key_type, int>::CanonicalSegment &cs)
  ...
  friend inline bool operator<(const Leafpiece &p, const key_type &key) {
    return p.key < key;
  }
  friend inline bool operator<(const key_type &key, const Leafpiece &p) {
    return key < p.key;
  }
  friend inline bool operator<(const Leafpiece &p1, const Leafpiece &p2) {
    return p1.key < p2.key;
  }
  ...
  inline void update(
      typename internal::insertPWLF<key_type, int>::CanonicalSegment &cs) {
    startkey = cs.first;
    endkey = cs.last;
    auto [cs_slope, cs_intercept] = cs.get_floating_point_segment(startkey);
    if (cs_intercept > std::numeric_limits<decltype(intercept)>::max())
  ...
  inline pair<bool, unsigned long> is_in_leaf(const key_type &key,
    ...
    if (key <= this->endkey) {
      ...
      for (; slotlo <= slothi; slotlo++) {
        //                    if (slotlo < 0)
        //                    {
        //                        cout<< "my Lord, please help, i trust in You!"
        //                        << endl;
        //                    }
        if (key < this->slotkey[slotlo] || key >= this->startkey) {
          ...
          //                        cout<< "my Lord, please help, i trust in
          //                        You!" << endl;
        }
        //                    else{
        //                        // check in sort piece
...
struct FILMinsert<key_type, value_type>::Innerpiece {
  ...
  friend inline bool operator<(const Innerpiece &p, const key_type &key) {
    return p.startkey < key;
  }
  friend inline bool operator<(const key_type &key, const Innerpiece &p) {
    return key < p.startkey;
  }
  friend inline bool operator<(const Innerpiece &p1, const Innerpiece &p2) {
...
typedef pair<filmadatype::Leafpiece *, unsigned short int>
    filmadalrupair;  // lru 中value 的type ，是一个pair，first is
                     // 所属的leaf，second 是slot in leaf
typedef adalru::hashLRU<key_type, filmadatype::Leafpiece *,
                        adalru::Node<key_type, filmadatype::Leafpiece *> *>
    filmadalrutype;
typedef adalru::localLRU<unsigned short, key_type *> locallrutype;
typedef filmstorage::filmdisk<key_type> filmadadisk;
typedef filmstorage::filmmemory<key_type, key_type *, filmadatype,
                                filmadalrutype, filmadadisk>
    filmadamemory;
typedef filmadamemory::memoryusage memoryusage;
//    adalru::hashLRU <key_type,Leafpiece* , adalru::Node<key_type ,Leafpiece*>*
//    >

void runtimeevictkeytopage(filmadamemory *filmmem, filmadatype *filmindex,
                           filmadadisk *diskpage, access_stats *r_stats) {
  ...
  if (filmmem->evictPoss.size() != 0) {
    for (int i = 0; i < filmmem->evictPoss.size(); i++) {
      ...
      auto evictslotV =
          evictleaf->intrachain.poptail();  // the tail of the accessed leaf
      gettimeofday(&lt2, NULL);
      ltimeuse = (lt2.tv_sec - lt1.tv_sec) +
                 (double)(lt2.tv_usec - lt1.tv_usec) / 1000000.0;
      r_stats->lrutime += ltimeuse;

      filmmem->evictkeytoinpage(evictleaf->slotkey[evictslotV->key],
                                evictslotV->value, diskpage, writeevict);
    ...
    wdtimeuse = (wdt2.tv_sec - wdt1.tv_sec) +
                (double)(wdt2.tv_usec - wdt1.tv_usec) / 1000000.0;
    r_stats->wdisktime += wdtimeuse;
  }
}

double runtimeevictkeytopage2(filmadamemory *filmmem, double totalusemem,
                              filmadatype *filmindex, filmadadisk *diskpage,
                              access_stats *r_stats) {
  ...
  if (filmmem->evictPoss.size() != 0) {
    ...
    gettimeofday(&wdt1, NULL);
    for (unsigned int i = 0; i < batch_evict;
         i++)  // the number of records to be evicted in batches
    {
      // evict data, get the tail node, remove the tail of intrachain
      gettimeofday(&lt1, NULL);
      auto evictleaf = filmmem->lru->get_tail();
      ...
      ltimeuse += (lt2.tv_sec - lt1.tv_sec) +
                  (double)(lt2.tv_usec - lt1.tv_usec) / 1000000.0;
      auto writeevict = filmmem->evictkeytoinpage(
          evictleaf->slotkey[evictslotV->key], evictslotV->value, diskpage);
      evictleaf->slotflag[evictslotV->key] = false;
      evictleaf->slotdata[evictslotV->key] = writeevict;
      delete evictslotV;
      evictslotV = NULL;
...
template <class v_type>  // int bplustree:  pair<
                         // oribttype::leaf_node*,pair<short int,int>
class prepass_dict {
 public:
  int pagenum = 0;
  ...
  struct dict_find {
    ...
    inline dict_find() : flags() {}
  };

  void insert_dict(pageid_type pageid,
                   v_type keyinfo) {  // keyinfo(first: offset in page, evicted
                                      // posi pair<int,short>*)
    //            typename
    //            std::unordered_map<int,vector<v_type>>::const_iterator got =
    //            prepass.find(pageid);
    typename std::map<pageid_type, vector<v_type>>::const_iterator got =
        prepass.find(pageid);
    if (got == prepass.end()) {
  ...
  void insert_dict(pageid_type pageid, pageOff_type off,
                   pair<pageid_type, pageOff_type>
                       *evictposi) {  // keyinfo(first: offset in page, evicted
                                      // posi pair<int,short>*)
    //            typename
    //            std::unordered_map<int,vector<v_type>>::const_iterator got =
    //            prepass.find(pageid);
  ...
  dict_find find_in_dict(pageid_type pageid) {
    ...
    if (got == prepass.end()) {
      std::cout
          << "not find the key in prepass, my lovely Lord, waiting for you! "
          << endl;
      res.flags = false;
      return res;
    } else {
...
typedef prepass_dict<pair<filmadatype::Leafpiece *,
                          pair<lruOff_type, pair<pageid_type, pageOff_type> *>>>
    pre_dict;

// in the range query, judge whether the keys in each overlapped leaf in memory
// or in disk
void range_is_in_or_out(int start, int end, filmadatype::Leafpiece *curleaf,
                        std::vector<pair<key_type, key_type *>> *totalres,
                        pre_dict *dict, int sizevalue, filmadamemory *filmmem,
                        access_stats *r_stats) {
  ...
  for (int k = start; k < end; k++) {
    ...
    if (flag) {
      ...
      auto finddata =
          (adalru::Node<lruOff_type, key_type *> *)curleaf->slotdata[k];
      ...
      // // dict 中插入的是 pageid，所属leafpiece，（在leaf 中的slot
      // // 位置，保存evictposi的地址（ pointer ）——用于后期存储被移出key
      // // 的外存地址）
      //                 dict->insert_dict(pageid, pair<
      //                 filmadatype::Leafpiece*,pair<lruOff_type,pair<pageid_type
      //                 ,pageOff_type>* >
      //                 >(curleaf,pair<lruOff_type,pair<pageid_type
      //                 ,pageOff_type>*>(k,evictpos)));
...
int range_prepass(
    filmadamemory *filmmem, filmadatype::range_res_find *index_result,
    std::vector<pair<key_type, key_type *>> *result, pre_dict *dict,
    filmadatype *filmindex,
    access_stats
        *r_stats) {  // 将range query 由btree 得到的信息，进行怕热pass，遍历find
                     // leaf中的每一个leaf，如果slotdata.first = true,
                     // 则直接放入reslut 中，否则将其放入prepassdict 中

  int leafnum = index_result->findleafs.size();
  if (leafnum == 1) {  // all the keys in range belonging to a certain leaf
    range_is_in_or_out(index_result->firstslot, index_result->lastslot + 1,
                       index_result->findleafs[0], result, dict,
                       filmindex->valuesize, filmmem, r_stats);
  } else {  // the first leaf and last leaf need to be deal with seperately
    // deal with firstleaf
    // index_result->findleafs[0];
    range_is_in_or_out(index_result->firstslot,
                       index_result->findleafs[0]->slotkey.size(),
                       index_result->findleafs[0], result, dict,
                       filmindex->valuesize, filmmem, r_stats);
    // deal with the middle leafs
    for (int i = 1; i < leafnum - 1; i++) {
      range_is_in_or_out(0, index_result->findleafs[i]->slotkey.size(),
                         index_result->findleafs[i], result, dict,
                         filmindex->valuesize, filmmem, r_stats);
    }
    // deal with lastleaf
    // index_result->findleafs[leafnum-1];
    range_is_in_or_out(0, index_result->lastslot + 1,
                       index_result->findleafs[leafnum - 1], result, dict,
                       filmindex->valuesize, filmmem, r_stats);
  }
  return 0;
}
...
// 一次至少读取1024k 的数据
int range_read_from_disk(std::vector<std::pair<key_type, key_type *>> *result,
                         pre_dict *dict, filmadamemory *filmmem,
                         filmadatype *filmindex, filmadadisk *pagedisk,
                         access_stats *r_stats) {
  // 打开磁盘文件
  ...
  fd = open(pagedisk->filename.c_str(), O_RDWR | O_DIRECT, 0755);
  if (cross * pagedisk->pagesize <=
      131072)  // 262144 2048k 判断一下，如果最大page 和最小page，相差超过1024k
               // 131072，则采用seek的形式进行读取，
  {
    ...
    ret = pread(fd, buf, buf_size,
                static_cast<unsigned long>(
                    aoffset));  // 从 prepass 的第一个页开始，读取所有跨着的页
    gettimeofday(&rdt2, NULL);
    rdtimeuse = (rdt2.tv_sec - rdt1.tv_sec) +
    ...
    for (auto &v : dict->prepass) {
      ...
      unsigned long int offset =
          diff * (pagedisk->recordsize *
                  pagedisk->recordnum);  // 把当前页的数据 单独出来
      for (int k = 0; k < v.second.size();
           k++) {  // v.second.size()  是要从当前页中读取多少个data，
                   // v.second[i] 是每个data
                   // 的相关信息是一个pair，v.second[i].first 是所属的leaf，
                   // v.second[i].second。first是slot in leaf，second 是
                   // evictposi指向page id 和off
        ...
        // 将res 插入到curpiece 的intrachain，// 将被访问的record
        // 的数据放入内存中，即其所属的leaf，并修改其slotdata 的flag 为 true
        gettimeofday(&lt1, NULL);
    ...
    dtimeuse = (dt2.tv_sec - dt1.tv_sec) +
               (double)(dt2.tv_usec - dt1.tv_usec) / 1000000.0;
    ...
    gettimeofday(&dt1, NULL);
    for (auto &v : dict->prepass) {
      pageid_type pageid = v.first;  // v.first 是 pageid， v.second 是 这个page
                                     // 要读取的数据的信息，
      if (pageid == filmmem->inpage->pageid) {
        cout << " Jesus, Son of David, please have pity on me, read from "
                "in-memory page"
             << endl;
        key_type *inmemdata = filmmem->inpage->inmemdata;

        for (int vi = 0; vi < v.second.size();
             vi++) {  // v.second.size()  是要从当前页中读取多少个data，
                      // v.second[i] 是每个data
                      // 的相关信息是一个pair，v.second[i].first 是所属的leaf，
                      // v.second[i].second。first是slot in leaf，second 是
                      // evictposi指向page id 和off
          auto slot =
              v.second[vi].second.first;  // v.first 是 page id，v.second
                                          // 是要从该页中读进来的每个record
                                          // 的信息，是一个pair，
          // 该pair 的first 是所属的leaf，该pair 的second 是一个pair，
          // 包括：first 是在该leaf 中的slot，second 是page 信息，包括page id
          // 和在该page 中的offset
          ...
          if (res.first != curpiece->slotkey[slot]) {
            cout << "i need You, my Lord, Thank You for all the thing! "
                 << endl;
          }
          // 将res 插入到curpiece 的intrachain，// 将被访问的record
          // 的数据放入内存中，即其所属的leaf，并修改其slotdata 的flag 为 true
          ...
          ltimeuse += (lt2.tv_sec - lt1.tv_sec) +
                      (double)(lt2.tv_usec - lt1.tv_usec) / 1000000.0;
          //                      cout<< " i need You, my Lord!!! we all need
          //                      You! "<< endl;
        }
      } else {
        ...
        // buf 中为所有属于这个页的数据，根据v.second 包含的offset in page
        // 的信息，将对应的值读出来，并放入leaf 中
        for (int vi = 0; vi < v.second.size();
             vi++) {  // v.second.size()  是要从当前页中读取多少个data，
                      // v.second[i] 是每个data
                      // 的相关信息是一个pair，v.second[i].first 是所属的leaf，
          ...
          ltimeuse += (lt2.tv_sec - lt1.tv_sec) +
                      (double)(lt2.tv_usec - lt1.tv_usec) / 1000000.0;
          //                cout<< " i need You, my Lord!!! we all need You! "<<
          //                endl;
        }
      }
    }
...
// excute point query
pair<access_stats, std::vector<std::vector<key_type>>> filmadapointquery(
    filmadatype *filmindex, std::vector<key_type> pqueries, int queryn,
    filmadadisk *pagedisk, filmadamemory *filmmem) {
  struct timeval qt1, qt2;
  double timeuse;
  std::vector<std::vector<key_type>> totalres;
  access_stats point_stats;
  pair<bool, double> transflag = filmmem->runtimejudgetrans(&point_stats);
  int periodV = filmmem->reserveMem * 1024 * 1024 /
                ((filmindex->valuesize + 1) * sizeof(key_type) * 10);
  ...
  for (unsigned int i = 0; i < pqueries.size(); i++) {
    ...
    if (index_res.find == false) {
      ...
      if (index_res.flags) {  // 从内存中读数据
        point_stats.memnum += 1;
        auto finddata = (adalru::Node<lruOff_type, key_type *> *)
        ...
        auto writeevict =
            (pair<pageid_type, pageOff_type> *)index_res.findleaf
                ->slotdata[index_res.slot];  // writeevict 指向的是 pageid and
                                             // offset
        if (writeevict->first == filmmem->inpage->pageid) {
          res.second = new key_type[filmindex->valuesize];
          key_type *inmemdata = filmmem->inpage->inmemdata;
        ...
        filmmem->lru->put(index_res.findleaf->startkey,
                          index_res.findleaf);  // 更新 interchain
        filmmem->evictPoss.emplace_back(writeevict);
        filmindex->inkeynum++;
        filmindex->exkeynum--;
      }
  ...
  timeuse = (qt2.tv_sec - qt1.tv_sec) +
            (double)(qt2.tv_usec - qt1.tv_usec) / 1000000.0;
  point_stats.querytimeuse += timeuse;
  point_stats.querytimeuse -= point_stats.computetimeuse;
  return std::pair<access_stats, std::vector<std::vector<key_type>>>(
      point_stats, totalres);
}

// interleave point and range queries
access_stats filmadaquery(filmadatype *filmindex,
                          std::vector<key_type> pointqueries,
                          std::vector<std::vector<key_type>> rangequeries,
                          unsigned int queryn, filmadadisk *pagedisk,
                          filmadamemory *filmmem) {
  struct timeval tqt1, tqt2, xt1, xt2, pt1, pt2, plt1, plt2, rdt1, rdt2;
  double pltimeuse, prdtimeuse, pwdtimeuse, timeuse,
      xtimeuse;  // 读磁盘的时间，写磁盘的时间
  ...
  for (unsigned int i = 0; i < querynum; i++) {
    ...
    //            if (querykey == 3176006677686915072 || querykey ==
    //            3004159460022892544 ||querykey ==  2919356911931049984
    //            ||querykey == 2831290469306449920){
    //                cout << "Jesus, i need You!" << endl;
    //            }
    // test_interleave_baselines(filename, memThreshold, datasize, numcolumn,
    // pagesize,errbnd);
    ...
    xtimeuse = (xt2.tv_sec - xt1.tv_sec) +
               (double)(xt2.tv_usec - xt1.tv_usec) / 1000000.0;
    query_stats.xlookuptime += xtimeuse;
    if (index_res.find == false) {  // 从 sort_piece 中 读取数据
      if (index_res.flags) {
        query_stats.memnum += 1;
        auto finddata = (adalru::Node<lruOff_type, key_type *> *)
                            filmindex->sort_list->slotdata[index_res.slot];
        res.first = querykey;
        res.second = finddata->value;

        gettimeofday(&plt1, NULL);
        filmindex->sort_list->intrachain.moveTohead(
            finddata);  // update intrachain
        //                totalres.emplace_back(res);
        // 更新 interchain
        filmmem->lru->put(filmindex->sort_list->slotkey[0],
                          filmindex->sort_list);
        gettimeofday(&plt2, NULL);
        pltimeuse = (plt2.tv_sec - plt1.tv_sec) +
                    (double)(plt2.tv_usec - plt1.tv_usec) / 1000000.0;
      ...
      if (index_res.flags) {  // 从内存中读数据
        ...
        gettimeofday(&plt1, NULL);
        index_res.findleaf->intrachain.moveTohead(
            finddata);  // update intrachain
        ...
        if (res.first != querykey) {
          cout << "not find, what's wrong? my Lord, i need You~~~~" << querykey
               << endl;
        }
        gettimeofday(&plt1, NULL);
        index_res.findleaf->slotdata[index_res.slot] =
    ...
    gettimeofday(&xt2, NULL);
    xtimeuse = (xt2.tv_sec - xt1.tv_sec) + (double) (xt2.tv_usec - xt1.tv_usec)
    / 1000000.0; query_stats.xlookuptime += xtimeuse; pre_dict *dict = new
    pre_dict(); range_prepass(
    filmmem,&index_result,&rres,dict,filmindex,&query_stats);
    ...
    if (dict->pagenum == 0){  // all the request data are in memory
        ...
        range_read_from_disk(&rres, dict, filmmem, filmindex,
    pagedisk,&query_stats);// read data from disk according to the information
    of prepass  根据prepass 的信息，从磁盘中读数据

        query_stats.disknum += 1;
        query_stats.diskpagenum += dict->pagenum;
        //cout << "i want in Your heart, my Lord!" << endl;
    }
    else{   // the request data are in both memory and disk 数据
    在内存和磁盘中都有 query_stats.crossnum += 1; query_stats.crosspagenum +=
    dict->pagenum;
  ...
  cout << "my Lord, You are my refuge~~~~forever! " << endl;
  timeuse = (tqt2.tv_sec - tqt1.tv_sec) +
            (double)(tqt2.tv_usec - tqt1.tv_usec) / 1000000.0;
...
int test_filmadaquery(unsigned int errbnd, size_t datanum, int pagesize,
                      string dataset, double mem_threshold, double reserveMem,
                      int recordSize, std::vector<key_type> keys,
                      std::vector<key_type> pointqueries,
                      std::vector<std::vector<key_type>> rangequeries,
                      unsigned int queryn, unsigned long numkey, int datasize,
  ...
  string file_str = diskpath + dataset + "_filmadapages" + ossd.str() + "_" +
                    ossm.str() + "_" + ossp.str() + "_" + ossr.str() + "_" +
                    osse.str();
  const char *diskfile = file_str.c_str();
  int numrecord = pagesize / (recordSize);
  filmstorage::filmmemory<key_type, key_type *, filmadatype, filmadalrutype,
                          filmadadisk>
      memoryfilm(numkey, mem_threshold, &filmada, &interchain);
  memoryfilm.reserveMem = reserveMem;
  filmstorage::filmdisk<key_type> diskfilm(file_str, pagesize, numrecord,
                                           recordSize);

  fstream fs;
  fs.open(file_str.c_str(), ios::in);
  if (fs) {
    remove(diskfile);
  }
  fs.close();
  ...
  for (auto i = 0; i < filmada.valuesize; i++) {
    payload[i] = i;
  }
  ...
  //        memoryfilm.insert(keys,payload,errbnd,errbnd,&diskfilm);    //
  //        执行数据插入
  memoryfilm.append(
      keys, payload, errbnd,
      errbnd);  // append data one by one 执行数据append ，逐个更新
  gettimeofday(&bt2, NULL);
  buildtimeuse = (bt2.tv_sec - bt1.tv_sec) +
                 (double)(bt2.tv_usec - bt1.tv_usec) / 1000000.0;
  ...
  savefile << "method "
           << "film_ada_lru "
           << "available_memory " << mem_threshold + reserveMem << " qtype "
           << workload << " error " << errbnd << " pagesize "
           << (pagesize * 8 / 1024) << "k recordsize ";
  savefile << recordSize << " build_time " << buildtimeuse << " dataset "
           << dataset << " datasize " << datasize << " keynum " << numkey
           << " ";
  savefile << flush;
  savefile.close();
  ...
  if (memoryfilm.inpage == NULL) {
    memoryfilm.createinmempage(diskfilm.pagesize,
                               diskfilm.recordnum);  // create a page in memory;
  }
  ...
  savefile << "method "
           << "film_ada_lru ";
  for (iter = transflag.second.meminfo.begin();
  ...
  access_stats query_performance = filmadaquery(
      &filmada, pointqueries, rangequeries, queryn, &diskfilm, &memoryfilm);

  query_performance.qtype = "query";
  query_performance.zipffactor = zipf;
  query_performance.workload = workload;
  cout << " Jesus, finished the range+point query test with film-adaptivelru, "
          "the performance is:"
       << endl;
...
int test_interleave_insert_query_baseline(
    unsigned int errbnd, size_t numkey, int pagesize, string dataset,
    double mem_threshold, double reserveMem, int recordSize,
    vector<key_type> keys, unsigned long actual_numkey, int datasize,
    double zipf, unsigned long init_num_keys, double insert_frac,
    double out_of_order_frac, unsigned long batch_size, string workload) {
  filmadatype filmada(0, errbnd, errbnd);
  filmadalrutype *interchain = new filmadalrutype(actual_numkey);
  filmada.valuesize = recordSize - 1;
  //        unsigned int num_out_inserts = 80000;   // 阈值：out_of_order
  //        insertion  160000/2, 160000, 80000*3, 160000*2
  std::ostringstream osse, ossr, ossd, ossm, ossp;
  osse << errbnd;
  ossp << pagesize;
  ossm << mem_threshold;
  ...
  memoryfilm.append(init_keys, payload, errbnd,
                    errbnd);  // 执行数据append ，逐个更新
  gettimeofday(&oribt2, NULL);
  buildtimeuse = (oribt2.tv_sec - oribt1.tv_sec) +
                 (double)(oribt2.tv_usec - oribt1.tv_usec) / 1000000.0;
  ...
  // evitct according to lru
  while (transflag.first) {
    initwritetime += memoryfilm.filmtransfer(
        transflag.second.meminfo["totalusage"], &diskfilm);
    transflag = memoryfilm.judgetransfer();
  }
  ...
  unsigned long int num_inserts_per_batch =
      static_cast<unsigned long>(batch_size * insert_frac);
  ...
  struct timeval workload_start_time, workload_run_time, insert_start_time,
      insert_end_time, lookup_start_time, lookup_end_time;
  double workload_elapsed_time = 0.0, lookup_elapsed_time = 0.0,
         insert_elapsed_time = 0.0, append_elapsed_time = 0.0;
  double time_limit = 3600;
  //        double querycumulative_writetime = 0.0;
  double cumulative_query_writetime = 0.0;
  double cumulative_insert_writetime =
      0;  // 记录插入process总共导致了多少 写磁盘的时间
  ...
  pair<bool, double> runtime_transflag =
      memoryfilm.runtimejudgetrans(&query_stats);
  int periodV = memoryfilm.reserveMem * 1024 * 1024 /
                ((filmada.valuesize + 1) * sizeof(key_type) * 10);

  while (true) {
    batch_no++;
    // generate the search key within the inserted key space
    if (i > 0) {
      //                unsigned int lookupnum = num_lookups_per_batch/2;  //
      //                divide by 2, due to one point and one range query
      unsigned int lookupnum =
          num_lookups_per_batch;  // divide by 2, due to one point and one range
                                  // query
      ...
      if (lookup_distribution == "random") {
        ...
        std::cerr << "--lookup_distribution must be either 'hotspot', "
                     "'randomzipf','random' or 'zipf'"
                  << std::endl;
        return 1;
      }
      ...
      for (unsigned int qi = 0; qi < lookupnum; qi++) {
        if (query_stats.disknum > 0 && qi % periodV == 0) {
          ...
          while (runtime_transflag.first) {
            batch_query_writetime +=
                runtimeevictkeytopage2(&memoryfilm, runtime_transflag.second,
        ...
        if (index_res.find == false) {  // 从 sort_piece 中 读取数据
          if (index_res.flags) {
            query_stats.memnum += 1;
            auto finddata = (adalru::Node<lruOff_type, key_type *> *)
                                filmada.sort_list->slotdata[index_res.slot];
            res.first = querykey;
            res.second = finddata->value;

            gettimeofday(&plt1, NULL);
            filmada.sort_list->intrachain.moveTohead(
                finddata);  // update intrachain
            //                totalres.emplace_back(res);
            // update golbal chain ( interchain)
            memoryfilm.lru->put(filmada.sort_list->slotkey[0],
                                filmada.sort_list);
            gettimeofday(&plt2, NULL);
            pltimeuse = (plt2.tv_sec - plt1.tv_sec) +
                        (double)(plt2.tv_usec - plt1.tv_usec) / 1000000.0;
            ...
            auto writeevict =
                (pair<pageid_type, pageOff_type> *)index_res.findleaf
                    ->slotdata[index_res.slot];  // writeevict 指向的是 pageid
                                                 // and offset
            res = diskfilm.odirectreadfromdisk(
                writeevict);  // if readfromdisk indicating doesn't use
                              // o_direct;
            //                totalres.push_back(res);
            index_res.findleaf->slotdata[index_res.slot] =
                index_res.findleaf->intrachain.put(index_res.slot, res.second);
            ...
            memoryfilm.lru->put(index_res.findleaf->startkey,
                                index_res.findleaf);  // 更新 interchain
          ...
          if (index_res.flags) {  // read data from memory    从内存中读数据
            ...
            auto writeevict =
                (pair<pageid_type, pageOff_type> *)index_res.findleaf
                    ->slotdata[index_res.slot];  // writeevict: pageid and
                                                 // offset
            ...
            if (writeevict->first == memoryfilm.inpage->pageid) {
              ...
              res = diskfilm.odirectreadfromdisk(writeevict);
            }
...
//                    if (lastkey == 19.271010892963488 || lastkey
== 19.271038535787397)
//                        cout<< "Jesus, please have pity on me"<< endl;

        gettimeofday(&xt1, NULL);
        auto index_result = filmada.search_range(firstkey, lastkey,
&query_stats); gettimeofday(&xt2, NULL);

        xtimeuse = (xt2.tv_sec - xt1.tv_sec) + (double) (xt2.tv_usec -
xt1.tv_usec) / 1000000.0; query_stats.xlookuptime += xtimeuse; pre_dict *dict =
new pre_dict(); range_prepass(&memoryfilm, &index_result, &rres, dict, &filmada,
&query_stats);
        // 判断 是否需要访问磁盘
...
&query_stats);
      ...
      lookup_elapsed_time =
          (lookup_end_time.tv_sec - lookup_start_time.tv_sec) +
          (double)(lookup_end_time.tv_usec - lookup_start_time.tv_usec) /
              1000000.0;
      ...
      append_elapsed_time =
          (insert_end_time.tv_sec - insert_start_time.tv_sec) +
          (double)(insert_end_time.tv_usec - insert_start_time.tv_usec) /
              1000000.0;
      ...
      while (transflag.first) {
        ...
        batch_insert_writetime +=
            runtimeevictkeytopage2(&memoryfilm, transflag.second.totalusemem,
                                   &filmada, &diskfilm, &query_stats);
        transflag = memoryfilm.judgetransfer(&query_stats);
      }

      // insert elapsed time  including the time of eviction
      ...
      workload_elapsed_time =
          (workload_run_time.tv_sec - workload_start_time.tv_sec) +
          (double)(workload_run_time.tv_usec - workload_start_time.tv_usec) /
              1000000.0;
      //            if (workload_elapsed_time > time_limit && (filmada.inkeynum
      //            + filmada.exkeynum) > actual_numkey ) {
      //                break;
      //            }
      ...
      if (print_batch_stats) {
        ...
        long long cumulative_operations =
            cumulative_lookups + cumulative_inserts;
        ...
        std::cout
            ...
            << (num_actual_inserts) / (append_elapsed_time) << " appends/sec,\t"
            << num_batch_operations / batch_time << " ops/sec"
            << num_batch_operations / (batch_time) << " ops_with_evict/sec"
            << "\n\tcumulative_throughput:\t"
            << cumulative_lookups / cumulative_lookup_time << " lookups/sec,\t"
        ...
        std::cout << "\n\tcumulative_execution_1:\t"
                  << (filmada.inkeynum + filmada.exkeynum) *
                         (filmada.valuesize + 1) * 8 / double(1024 * 1024)
                  << " G datasize, \t" << filmada.inkeynum + filmada.exkeynum
                  << " #cumulative_keys,\t" << filmada.inkeynum
                  << " #inmem_keys,\t" << filmada.exkeynum << " #exmem_keys"
                  << "\n\tcumulative_execution_1:\t" << cumulative_updates
                  << " #cumulative_updates,\t" << cumulative_inserts
                  << " #cumulative_inserts,\t" << cumulative_insert_time
                  << " cumulative_insert_time,\t" << cumulative_lookups
                  << " #cumulative_lookups,\t" << cumulative_lookup_time
                  << " cumulative_lookup_time,\t"
                  << (diskfilm.initwtime / 1000000.0) + query_stats.wdisktime
                  << " cumulative_writetime " << cumulative_insert_writetime
                  << " cumulative_insert_writetime ";
        ...
        savefile << batch_no << " Batch " << cumulative_operations
                 << " cumulative_ops "
                 << "_"
                 << " batch_throughput "
                 ...
                 << " inserts/sec "
                 << (num_actual_inserts - num_actual_updates) /
                        append_elapsed_time
                 << " appends/sec " << num_batch_operations / batch_time
                 << " ops/sec " << batch_query_writetime
                 << " batch_query_writetime " << batch_insert_writetime
                 << " batch_insert_writetime "
                 << "_"
                 << " cumulative_throughput "
                 << cumulative_lookups / cumulative_lookup_time
        ...
        savefile << (filmada.inkeynum + filmada.exkeynum) *
                        (filmada.valuesize + 1) * 8 / double(1024 * 1024)
                 << "G datasize " << filmada.inkeynum + filmada.exkeynum
                 << " #cumulative_keys " << filmada.inkeynum << " #inmem_keys "
                 << filmada.exkeynum << " #exmem_keys " << cumulative_updates
                 << " #cumulative_updates " << cumulative_inserts
                 << " #cumulative_inserts " << cumulative_insert_time
                 << " cumulative_insert_time " << cumulative_lookups
                 << " #cumulative_lookups " << cumulative_lookup_time
      ...
      if ((cumulative_lookup_time + cumulative_insert_time) > time_limit ||
          (filmada.inkeynum + filmada.exkeynum) >= numkey) {
        break;
      }
    ...
    cout << "my lovely Lord, finished the interleave inserts and queries of "
            "batch "
         << batch_no << endl;
  }
  fs.open(diskfile, ios::in);
...
int test_interleave_insert_query(unsigned int errbnd, size_t numkey,
                                 int pagesize, string dataset,
                                 double mem_threshold, double reserveMem,
                                 int recordSize, vector<key_type> keys,
                                 unsigned long actual_numkey, int datasize,
                                 double zipf, unsigned long init_num_keys,
                                 double insert_frac, double out_of_order_frac,
                                 unsigned long batch_size, string workload,
                                 unsigned int threshold) {
  filmadatype filmada(0, errbnd, errbnd);
  filmadalrutype *interchain = new filmadalrutype(actual_numkey);
  filmada.valuesize = recordSize - 1;
  unsigned int num_out_inserts =
      threshold;  // 阈值：out_of_order insertion  160000/2, 160000, 80000*3,
                  // 160000*2
  std::ostringstream osse, ossr, ossd, ossm, ossp;
  osse << errbnd;
  ossp << pagesize;
  ...
  while (transflag.first) {
      if (filmada.leafsplit == 0) {
          ...
          transleaves = (leaves - filmada.leafsplit) * (1 - ratio);  // the
  transfer number of leafs } else { ratio = memoryfilm.threshold /
  transflag.second.totalusemem; transleaves = (leaves - filmada.leafsplit) * (1
  - ratio); if (transleaves == 0) { if (errbnd >= 64) transleaves = 1; else
  transleaves = 3;
          }
...
*/
  ...
  struct timeval workload_start_time, workload_run_time, insert_start_time,
      insert_end_time, lookup_start_time, lookup_end_time;
  double workload_elapsed_time = 0.0, lookup_elapsed_time = 0.0,
         insert_elapsed_time = 0.0, update_elapsed_time = 0.0,
         append_elapsed_time = 0.0, rebuild_elapsed_time = 0.0;
  double time_limit = 3600;
  //        double querycumulative_writetime = 0.0;
  double cumulative_query_writetime = 0.0;
  double cumulative_insert_writetime =
  ...
  while (true) {
    ...
    // generate the search key within the inserted key space
    if (i > 0) {
      //                key_type maxinkey = init_keys[init_num_keys-1];// the
      //                maxkey  19.270965511316433
      key_type *lookup_keys = nullptr;  // generate lookup keys
      unsigned int lookupnum =
          num_lookups_per_batch /
          2;  // divide by 2, due to one point and one range query
      if (lookup_distribution == "random") {
        lookup_keys = get_search_keys(keys, i, lookupnum);
      } else if (lookup_distribution == "zipfrandom") {
        lookup_keys = get_search_keys_scrambledzipf(keys, i, lookupnum);
      ...
      for (unsigned int qi = 0; qi < lookupnum; qi++) {
        ...
        if (index_res.find == false) {  // 从 sort_piece 中 读取数据
          ...
          if (index_res.flags) {  // 从内存中读数据
            ...
            //                totalres.push_back(res);
            if (res.first != querykey) {
              cout << "not find, what's wrong? my Lord, i need You~~~~"
                   << querykey << endl;
            }
            gettimeofday(&plt1, NULL);
            index_res.findleaf->slotdata[index_res.slot] =
        ...
        gettimeofday(&xt1, NULL);
        auto index_result =
            filmada.search_range(firstkey, lastkey, &query_stats);
        gettimeofday(&xt2, NULL);
        ...
        range_prepass(&memoryfilm, &index_result, &rres, dict, &filmada,
                      &query_stats);
        ...
        if (dict->pagenum == 0) {  // the request data  数据都在内存
          ...
          range_read_from_disk(
              &rres, dict, &memoryfilm, &filmada, &diskfilm,
              &query_stats);  // read data from disk according to 根据prepass
                              // 的信息，从磁盘中读数据
      ...
      // randomly choose some keys to be inserted out-of-order
      unsigned long int num_actual_updates =
          num_actual_inserts * out_of_order_frac;
      std::vector<key_type> update_vec, append_vec;
      ...
      std::uniform_real_distribution<> dist(1,
                                            upper);  // define the range  [a, b)
      key_type maxinkey = keys[num_keys_after_batch - 1];
      if (out_of_order_frac == 0.0) {
        append_vec.assign(keys.begin() + i,
                          keys.begin() + num_keys_after_batch);
      ...
      /*
      // if out_of_order insertion, judge whether arrive at the threshold to
      rebuild
       */
      ...
      if (out_of_order_frac) {
        ...
        if (filmada.sort_list == NULL && update_vec.size() > num_out_inserts ||
          ...
          if (memoryfilm.inpage != NULL) {
            ...
            vector<pair<pageid_type, pageOff_type>>().swap(
                memoryfilm.evicttable);
            vector<pair<pageid_type, pageOff_type> *>().swap(
                memoryfilm.evictPoss);
            memoryfilm.evicttable.reserve(numkey);
            diskfilm.nextpageid = 0;
          }
          // record rebuild end time
          gettimeofday(&lookup_end_time, NULL);
          rebuild_elapsed_time =
              (lookup_end_time.tv_sec - lookup_start_time.tv_sec) +
              (double)(lookup_end_time.tv_usec - lookup_start_time.tv_usec) /
                  1000000.0;
      ...
      std::vector<key_type>().swap(update_vec);
      std::vector<key_type>().swap(append_vec);
      ...
      if (print_batch_stats) {
        ...
        std::cout
            ...
            << " appends/sec,\t" << num_actual_updates / update_elapsed_time
            ...
            << cumulative_lookups / cumulative_lookup_time << " lookups/sec,\t"
            << cumulative_inserts / cumulative_insert_time << " inserts/sec,\t"
            << cumulative_updates / cumulative_update_time << " updates/sec,\t"
            << cumulative_operations / cumulative_time << " ops/sec "
            << cumulative_query_writetime << " cumulative_query_writetime "
            << cumulative_insert_writetime << " cumulative_insert_writetime "
        ...
        std::cout << "\n\tcumulative_execution_1:\t"
                  ...
                  << " cumulative_update_time,\t" << cumulative_inserts
                  << " #cumulative_inserts,\t" << cumulative_insert_time
                  << " cumulative_insert_time,\t" << cumulative_lookups
                  << " #cumulative_lookups,\t" << cumulative_lookup_time
                  << " cumulative_lookup_time,\t"
                  << (diskfilm.initwtime / 1000000.0) + query_stats.wdisktime
                  << " cumulative_writetime " << cumulative_insert_writetime
                  << " cumulative_insert_writetime " << update_elapsed_time
                  << " update_elapsed_time " << cumulative_update_time
                  << " cumulative_update_time " << rebuild_elapsed_time
                  << " rebuild_elapsed_time ";
        ...
        savefile << (filmada.inkeynum + filmada.exkeynum) *
                 ...
                 << " #cumulative_keys " << filmada.inkeynum << " #inmem_keys "
                 << filmada.exkeynum << " #exmem_keys " << cumulative_updates
                 << " #cumulative_updates " << retrain_updates
                 << " #retrain_updates " << cumulative_update_time
                 << " cumulative_update_time " << cumulative_inserts
                 << " #cumulative_inserts " << cumulative_insert_time
                 << " cumulative_insert_time " << cumulative_lookups
                 << " #cumulative_lookups " << cumulative_lookup_time
      ...
      // Check for workload end conditions
      if (workload_elapsed_time > time_limit ||
          (filmada.inkeynum + filmada.exkeynum) >= numkey) {
        break;
      }
