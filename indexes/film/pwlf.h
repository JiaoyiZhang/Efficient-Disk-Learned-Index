...
//            throw std::logic_error("Points must be increasing by x.");
        ...
        inline bool append_point(const X &x, const Y &y) {
//        if (points_in_hull > 0 && x <= last_x)
//            throw std::logic_error("Points must be increasing by x.");
...
//            }
        ...
        inline CanonicalSegment get_segment() {
            if (points_in_hull == 1)
                return CanonicalSegment(rectangle[0], rectangle[1], first_x);
            return CanonicalSegment(rectangle, first_x);
        }

        inline CanonicalSegment get_segment( X break_x) {
            if (points_in_hull == 1)
                return CanonicalSegment(rectangle[0], rectangle[1], first_x);
            return CanonicalSegment(rectangle, first_x,break_x);
        }

        inline CanonicalSegment get_segment( X break_x,std::vector<X> keys_vec) {
            if (points_in_hull == 1)
                return CanonicalSegment(rectangle[0], rectangle[1], first_x);
    ...
    public:
        ...
        inline std::pair<long double, long double> get_intersection() const {
            auto &p0 = rectangle[0];
            auto &p1 = rectangle[1];
            auto &p2 = rectangle[2];
            auto &p3 = rectangle[3];
        ...
        inline std::pair<long double, SY> get_floating_point_segment(const X &origin) const {
            if (one_point())
        ...
        inline std::pair<long double, long double> get_slope_range() const {
            if (one_point())
                return {0, 1};
...
//        key_type* value = new key_type[valuesize];
            auto putres=filmada->m_tailleaf->intrachain.put(p.second,payload);
        filmada->m_tailleaf->slotdata.emplace_back(putres);
        filmada->m_tailleaf->slotkey.emplace_back(p.first);
        filmada->m_tailleaf->slotflag.emplace_back(true);
        for (; inkeynum < keys.size(); ++inkeynum) {
...
//                startkeys.clear();
                std::vector<key_type>().swap(startkeys);
//                cout << "Jesus, i need You!!"<< endl;
                pos = 0;
