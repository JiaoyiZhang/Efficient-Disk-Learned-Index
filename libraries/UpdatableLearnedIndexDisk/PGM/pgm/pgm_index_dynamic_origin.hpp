...
#define BLOCK_RECORDING 1
  ...
  void read_block_or_not(int file_handler, int32_t *last_block,
  ...
  ItemOnDisk obtain_item_on_disk(size_t pos, int file_handler, int *last_block,
                                 void *block_data) {
    ...
    ItemOnDisk *_it_seg =
        (ItemOnDisk *)(block_data + sizeof(ItemOnDisk) * item_offset);
    return {_it_seg->key, _it_seg->value};
  }

  bool update_item_on_disk(size_t pos, ItemOnDisk new_item, int file_handler,
                           void *block_data) {
    int block_to_fetch = -1;
    int item_offset = -1;
    item_block_offset(pos, &block_to_fetch, &item_offset);
    ...
    write_block(file_handler, block_data, block_to_fetch);
    return true;
  }

  size_t lower_bound_bl_disk(int file_handler, size_t first, size_t last,
    ...
    while (n > 1) {
      auto half = n / 2;
      first = obtain_item_on_disk(first + half, file_handler, &last_block, buf)
                          .key < x
                  ? first + half
                  : first;
      n -= half;
    ...
    size_t pos =
        first +
        (obtain_item_on_disk(first, file_handler, &last_block, buf).key < x);
    if (pos != N) {
      *item = obtain_item_on_disk(pos, file_handler, &last_block, buf);
    }
    // delete []block_data;
    return pos;
  }
  ...
  void pairwise_merge_on_disk(ItemOnDisk new_item, uint8_t target,
    ...
    // read the data in min_level
    {
      ...
      for (auto i = 0; i < block_count; i++) {
        read_block(get_data_file_handler(min_level), buf, i);
    ...
    for (uint8_t i = 1 + min_level; i <= merge_limit;
...

      {
        ...
        for (auto bi = 0; bi < block_count; bi++) {
          read_block(get_data_file_handler(i), buf, bi);
          int cp_size = ItemCountPerBlock;
    ...
    for (; start < tmp_size;) {
      ...
      write_block(get_data_file_handler(target), buf, block);
    ...
    if (has_pgm(target)) {
      ...
      pgm(target) = PGMType(item_vec.begin(), item_vec.end(), inner_disk,
                            get_index_file_handler(target), epsilon_value);
    }
  ...
  void insert_item_on_file(int level, size_t position, ItemOnDisk item) {
    ...
    if (position == item_count) {
      if (offset == 0) {  // we just write a block at block_id
        ...
        write_block(file_handler, buf, block_id);
      } else {  // read the original block and add the new item
        read_block(file_handler, buf, block_id);
    ...
    for (auto i = block_id; i < block_end; i++) {
      ...
      if (i == block_id) {
        ...
        memcpy(items + cp_count, buf + ItemOnDiskSize * offset,
    ...
    for (; start < cp_count;) {
      ...
      write_block(file_handler, buf, block);
      start += ItemCountPerBlock;
      block += 1;
    }
  ...
  void insert_on_disk_internal(ItemOnDisk new_item, long long *search_latency,
                               long long *insert_latency,
                               long long *smo_latency, int *smo_count,
                               int *updated_tuples) {
#if Profiling
    *smo_count = 0;
    *updated_tuples = 0;
    ...
    std::chrono::high_resolution_clock::time_point smo_start =
        std::chrono::high_resolution_clock::now();
    ...
    auto insertion_point =
        lower_bound_bl_disk(get_data_file_handler(min_level), 0, item_count,
...
#if Profiling
    *search_latency = std::chrono::duration_cast<std::chrono::nanoseconds>(
                          search_end - search_start)
                          .count();
    *insert_latency = std::chrono::duration_cast<std::chrono::nanoseconds>(
                          insert_end - insert_start)
                          .count();
    *smo_latency = std::chrono::duration_cast<std::chrono::nanoseconds>(
                       smo_end - smo_start)
                       .count();
#endif
    return;
 ...
 public:
  ...
  V find_on_disk(const K &key, int *c, int *ic, int *lc) {
    ...
    for (auto i = min_level; i < used_levels; ++i) {
      ...
      if (has_pgm(i)) {
        // pgm search on disk
        auto range = inner_disk ? pgm(i).search_disk(get_index_file_handler(i),
                                                     key, c, lc)
                                : pgm(i).search(key);
        first = range.lo;
        last = range.hi;
      ...
      *ic = *c;
      auto pos = lower_bound_bl_disk(get_data_file_handler(i), first, last, key,
                                     item_count, &item);
  ...
  int range_on_disk(const K &lo, const K &hi, int len, int *c,
    ...
    for (auto i = min_level; i < used_levels; i++) {
      ...
      if (has_pgm(i)) {
        ...
        ItemOnDisk _item;
        lo_first = lower_bound_bl_disk(get_data_file_handler(i), lo_first,
      ...
      while (l_i <= tmp_size && r_i < item_count_i && !is_out_of_range &&
        ...
        int last_block = -1;
        read_block_or_not(get_data_file_handler(i), &last_block, block_id, buf);
  ...
  void insert_on_disk(K key, V value, long long *search_latency,
    ...
    x.key = key;
    x.value = value;
    insert_on_disk_internal(x, search_latency, insert_latency, smo_latency,
                            smo_count, updated_tuples);
  }
 ...
 public:
   ...
   * speed up searches
   */
  DynamicPGMIndexOrigin(size_t eps, uint8_t base = 8, uint8_t buffer_level = 0,
  ...
  DynamicPGMIndexOrigin(Iterator first, Iterator last, size_t eps,
                        uint8_t base = 8, uint8_t buffer_level = 0,
                        uint8_t index_level = 0)
      : DynamicPGMIndexOrigin(eps, base, buffer_level, index_level) {
    ...
    *out++ = Item(first->first, first->second);
    ...
    if (has_pgm(used_levels - 1)) {
      ...
      pgm(used_levels - 1) =
          PGMType(target.begin(), target.end(), epsilon_value);
    }
  }
   ...
   */
  template <typename Iterator>
  DynamicPGMIndexOrigin(bool is_first_time, bool _inner_disk, Iterator first,
                        Iterator last, size_t eps, uint8_t base = 8,
...
class DynamicPGMIndexOrigin<K, V, PGMType>::Iterator {
  friend class DynamicPGMIndexOrigin;

  using level_iterator = typename Level::const_iterator;
  using dynamic_pgm_type = DynamicPGMIndexOrigin<K, V, PGMType>;
...
template <typename K, typename V, typename PGMType>
class DynamicPGMIndexOrigin<K, V, PGMType>::ItemA {
...
template <typename K, typename V, typename PGMType>
const V DynamicPGMIndexOrigin<K, V, PGMType>::ItemA::tombstone =
    get_tombstone<V>();

template <typename K, typename V, typename PGMType>
class DynamicPGMIndexOrigin<K, V, PGMType>::ItemB {
  bool flag;
