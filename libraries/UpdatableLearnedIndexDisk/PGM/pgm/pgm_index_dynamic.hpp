...
#include "pgm_index_dynamic_origin.hpp"
#include <cstddef>
#include <cstdint>
#include <algorithm>
#include <cassert>
...
class DynamicPGMIndex {
    ...
    typedef struct {
        ...
        bool data_in_memory[12];
        size_t memory_budget;
        int32_t last_memory_level;
        int32_t last_memory_level_count;
    } Metadata;
...
private:
    ...
    Metadata metadata;
    std::vector<std::vector<ItemOnDisk>> memory_data; // only support find, update and insert operations
    ...
    void initial_metadata() {
        ...
        for (auto i = 0; i < 12; i++) {
            ...
            metadata.data_in_memory[i] = false;
            memory_data.push_back(std::vector<ItemOnDisk>());
        }
    }

    void init_memory_buffer(){
        int sum_size = 0;
        int max_memory_size = std::ceil(metadata.memory_budget * 1.0 / (sizeof(K) + sizeof(V)));
        for (auto i = min_level; i < 12; i++) {
            int tmp_buf_size = sum_size;
            sum_size += max_size(i);
            if (sum_size <= max_memory_size) {
                metadata.data_in_memory[i-min_level] = true;
                if(i==min_level){
                    memory_data[i-min_level].resize(buffer_max_size, {std::numeric_limits<K>::max(), 0});
                    int block_num =std::ceil(buffer_max_size*1.0 / ItemCountPerBlock);
                    memory_data[i-min_level].reserve(block_num * ItemCountPerBlock);
                } else {
                    memory_data[i-min_level].resize(max_size(i), {std::numeric_limits<K>::max(), 0});
                    int block_num =std::ceil(max_size(i)*1.0 / ItemCountPerBlock);
                    memory_data[i-min_level].reserve(block_num * ItemCountPerBlock);
                }
            } else {
                metadata.data_in_memory[i-min_level] = true;
                metadata.last_memory_level = i;
                metadata.last_memory_level_count = max_memory_size - tmp_buf_size;
                std::cout<<i<<",\tmetadata.last_memory_level "<<metadata.last_memory_level <<",\tcnt:"<<metadata.last_memory_level_count<<std::endl;

                memory_data[i-min_level].resize(metadata.last_memory_level_count, {std::numeric_limits<K>::max(), 0});
                int block_num = std::ceil(metadata.last_memory_level_count*1.0 / ItemCountPerBlock);
                memory_data[i-min_level].reserve(block_num * ItemCountPerBlock);
                break;
            }
        }
    ...
    void load_metadata() {
        ...
        read_block(metadata_handler, buf, 0);
    ...
    void flush_metadata() {
        ...
        write_block(metadata_handler, buf, 0);
        // delete []block;
        return;
    }
    ...
    bool get_data_position(int level) {
        if (level > 11) return -1;
        return metadata.data_in_memory[level-min_level];
    }
    ...
    void create_related_empty_files() {
        for (auto i = 0; i < 12 - min_level; i++) {
            ...
            int fd = open(name.c_str(), O_RDWR | O_CREAT | O_DIRECT, 0644);
            close(fd);
        }
    ...
    void read_block_or_not(int file_handler, int32_t *last_block, int32_t block_to_fetch, void *block_data, bool in_memory, ItemOnDisk* level_memory_data) {
            if (block_to_fetch != *last_block) {
                read_block<ItemOnDisk>(file_handler, block_data, block_to_fetch, in_memory, level_memory_data);
    ...
    ItemOnDisk obtain_item_on_disk(size_t pos, int file_handler, int *last_block, void *block_data, bool in_memory, ItemOnDisk* level_memory_data) {
            int block_to_fetch = -1;
            int item_offset = -1;
            item_block_offset(pos, &block_to_fetch, &item_offset);
            read_block_or_not(file_handler, last_block, block_to_fetch, block_data, in_memory, level_memory_data);
            ItemOnDisk *_it_seg = (ItemOnDisk *)(block_data + sizeof(ItemOnDisk) * item_offset);
            return {_it_seg->key, _it_seg->value};
        }

    bool update_item_on_disk(size_t pos, ItemOnDisk new_item, int file_handler, void *block_data, bool in_memory, ItemOnDisk* level_memory_data) {
            int block_to_fetch = -1;
            int item_offset = -1;
            item_block_offset(pos, &block_to_fetch, &item_offset);
            ((ItemOnDisk*)((char *)block_data + sizeof(ItemOnDisk) * item_offset))->key = new_item.key;
            ((ItemOnDisk*)((char *)block_data + sizeof(ItemOnDisk) * item_offset))->value = new_item.value;
            write_block<ItemOnDisk>(file_handler, block_data, block_to_fetch, in_memory, level_memory_data);
            return true;
        }

    size_t lower_bound_bl_disk(int file_handler, size_t first, size_t last, const K &x, size_t N, ItemOnDisk *item, bool in_memory, ItemOnDisk* level_memory_data) {
            int last_block = -1;
            ...
            while (n > 1) {
                auto half = n / 2;
                first = obtain_item_on_disk(first+half, file_handler, &last_block, buf, in_memory, level_memory_data).key < x
                        ? first + half : first;
                n -= half;
            }
            // the second part should have a lower overhead
            size_t pos = first + (obtain_item_on_disk(first, file_handler, &last_block, buf, in_memory, level_memory_data).key < x);
            if (pos != N) {
                *item = obtain_item_on_disk(pos, file_handler, &last_block, buf, in_memory, level_memory_data);
            }
            // delete []block_data;
            return pos;
        }
    ...
    void pairwise_merge_on_disk(ItemOnDisk new_item, uint8_t target,
        ...
        // read the data in min_level
        {
            ...
            for (auto i = 0; i < block_count; i++) {
                read_block<ItemOnDisk>(get_data_file_handler(min_level), buf, i, metadata.data_in_memory[0], &(memory_data[0][0]));
        ...
        for (uint8_t i = 1 + min_level; i <= merge_limit; ++i, alternate = !alternate) {
...

            {
                ...
                for (auto bi = 0; bi < block_count; bi++) {
                    read_block<ItemOnDisk>(get_data_file_handler(i), buf, bi, metadata.data_in_memory[i-min_level], &(memory_data[i-min_level][0]));
                    int cp_size = ItemCountPerBlock;
...
//                }
        ...
        for (;start < tmp_size;) {
            ...
            write_block<ItemOnDisk>(get_data_file_handler(target), buf, block, metadata.data_in_memory[target-min_level], &(memory_data[target-min_level][0]));
...
//                                      :  std::vector<ItemOnDisk> (tmp_b, tmp_b + sizeof(tmp_b));
    ...
    void insert_item_on_file(int level, size_t position, ItemOnDisk item) {
        ...
        if (position == item_count) {
            if (offset == 0) { // we just write a block at block_id
                ...
                write_block<ItemOnDisk>(file_handler, buf, block_id, metadata.data_in_memory[level-min_level], &(memory_data[level-min_level][0]));
            } else { // read the original block and add the new item
                read_block<ItemOnDisk>(file_handler, buf, block_id, metadata.data_in_memory[level-min_level], &(memory_data[level-min_level][0]));
        ...
        for (auto i = block_id; i < block_end; i++) {
            read_block<ItemOnDisk>(file_handler, buf, i, metadata.data_in_memory[level-min_level], &(memory_data[level-min_level][0]));
            if (i == block_id) {
                memcpy(items, buf, ItemOnDiskSize * offset);
                items[offset] = item;
                cp_count = offset + 1;
                int remain = i == block_end - 1 ? item_in_last_block - offset : ItemCountPerBlock - offset;
                memcpy(items + cp_count, buf + ItemOnDiskSize * offset, ItemOnDiskSize * remain);
                cp_count += remain;
                continue;
            }
            memcpy(items + cp_count, buf, ItemOnDiskSize * ( i == block_end - 1 ? item_in_last_block : ItemCountPerBlock));
        ...
        for (;start < cp_count;) {
            ...
            write_block<ItemOnDisk>(file_handler, buf, block, metadata.data_in_memory[level-min_level], &(memory_data[level-min_level][0]));
            start += ItemCountPerBlock;
            block += 1;
        }
    ...
    // bool insert_into_memory(ItemOnDisk new_item){
    //     if(memory_data[0].size() < metadata.memory_max_count[min_level]){
    //         memory_data[0].insert({new_item.key, new_item.value});
    //         return true;
    //     }
    //     int slots_required = 1;
    //     int i = 0;
    //     for(; i<memory_data.size(); i++){
    //         auto item_count_i = memory_data[i].size();
    //         auto slots_left_in_level = metadata.memory_max_count[i-min_level] - item_count_i;
    //         if (slots_required <= slots_left_in_level)
    //             break;
    //         slots_required += item_count_i;
    //     }
    //     if (i == memory_data.size()) {
    //         move_memory_to_disk();
    //         memory_data[0].insert({new_item.key, new_item.value});
    //     } else {
    //         for (int j=i-1; j>=0; j--){
    //             memory_data[i].insert(memory_data[j].begin(), memory_data[j].end());
    //             memory_data[j].clear();
    //         }
    //         memory_data[i].insert({new_item.key, new_item.value});
    //     }
    //     return true;
    // }

    // void move_memory_to_disk(){
    //     auto it = memory_data[0].begin();
    ...
    void insert_on_disk_internal(ItemOnDisk new_item, long long *search_latency,
        ...
        *updated_tuples = 0;
        ...
        auto insertion_point = lower_bound_bl_disk(get_data_file_handler(min_level),
                                                   0, item_count,
                                                   new_item.key, item_count, &item, metadata.data_in_memory[0], &(memory_data[0][0]));
#if Profiling
...
public:
    ...
    size_t report_main_memory_size() {
        ...
        for (auto i = min_level; i < used_levels; ++i) {
            if (metadata.data_in_memory[i-min_level]){
                total_size += memory_data[i-min_level].size()*sizeof(ItemOnDisk);
            }
            auto item_count = get_item_count(i);
            if (item_count == 0) continue;
            if (has_pgm(i)) {
    ...
    V find_on_disk(const K &key, int *c, int *ic, int *lc) {
        ...
        ItemOnDisk item;
            for (auto i = min_level; i < used_levels; ++i) {
                ...
                *ic = *c;
                auto pos = lower_bound_bl_disk(get_data_file_handler(i), first, last, key, item_count, &item, metadata.data_in_memory[i-min_level], &(memory_data[i-min_level][0]));
    ...
    bool update_on_disk(const K &key, const V &value, int *c, int *ic, int *lc) {
        ACCESSED_BLOCK_COUNT_Data = 0;
        ItemOnDisk item;
            for (auto i = min_level; i < used_levels; ++i) {
                auto item_count = get_item_count(i);
                ...
                *c += ACCESSED_BLOCK_COUNT_Data;
                if (pos != item_count && item.key == key) {
                    ...
                    update_item_on_disk(pos, new_item, get_data_file_handler(i), buf, metadata.data_in_memory[i-min_level], &(memory_data[i-min_level][0]));
                    return true;
                }
            }
            return false;
        ...
        ACCESSED_BLOCK_COUNT_Data = 0;
            ...
            for (auto i = min_level; i < used_levels; i++) {
                ...
                if (has_pgm(i)) {
                    ...
                    ItemOnDisk  _item;
                    lo_first = lower_bound_bl_disk(get_data_file_handler(i), lo_first, lo_last, lo, lo_last, &_item, metadata.data_in_memory[i-min_level], &(memory_data[i-min_level][0]));
                }
                ...
                while (l_i <= tmp_size && r_i < item_count_i && !is_out_of_range && offset < ItemCountPerBlock) {
                    int last_block = -1;
                    read_block_or_not(get_data_file_handler(i), &last_block, block_id, buf, metadata.data_in_memory[i-min_level], &(memory_data[i-min_level][0]));
...
private:
    ...
    size_t max_size(uint8_t level) const { 
        if (level == metadata.last_memory_level){
            return metadata.last_memory_level_count;
        }
...
public:
    ...
    DynamicPGMIndex(bool is_first_time, bool _inner_disk, Iterator first, Iterator last, uint8_t base = 8, uint8_t buffer_level = 0, uint8_t index_level = 0)
        ...
        *out++ = Item(first->first, first->second);
        ...
        for (;start < target.size();) {
            ...
            write_block<ItemOnDisk>(get_data_file_handler(used_levels-1), buf, block, metadata.data_in_memory[used_levels-1-min_level], &(memory_data[used_levels-1-min_level][0]));
            start += ItemCountPerBlock;
            block += 1;
        }
        // delete []block_data;
    ...
    template<typename Iterator>
    void build(size_t mem_budget, std::string path, bool is_first_time, bool _inner_disk, Iterator first, Iterator last, uint8_t base = 8, uint8_t buffer_level = 0, uint8_t index_level = 0) {
        suffix = path;
        ...
        *out++ = Item(first->first, first->second);
        ...
        metadata.memory_budget = mem_budget - report_main_memory_size();
        init_memory_buffer();
    }
