...
#include "storage_utility_direct_io.hpp"
...
//            int x = 0;
//        }
        Segment *it_seg = (Segment *)(buf + sizeof(Segment) * item_offset);
        ...
        Segment *it_next_seg = (Segment *)(buf + sizeof(Segment) * next_offset);
    ...
    size_t segment_for_key_disk(int file_handler, const K &key, int *lc) {
...
public:
    ...
    template<typename RandomIt>
    PGMIndex(RandomIt first, RandomIt last, bool _inner_disk, int file_handler, int eps)
        : n(std::distance(first, last)),
        first_key(n ? *first : K(0)),
        segments(),
        levels_offsets(),
    ...
    PGMIndex(int file_handler, bool inner_disk, size_t eps)
    : epsilon_value(eps) {
    ...
    void load_file(int file_handler, bool inner_disk) {
        // char *block_data = new char[BLOCK_SIZE];
        read_block(file_handler, buf, 0);
        int offset = 0;
        memcpy(&first_key, buf + offset, sizeof(K));
        offset += sizeof(K);
        memcpy(&n, buf + offset, sizeof(size_t));
        offset += sizeof(size_t);
        size_t level_size;
        memcpy(&level_size, buf + offset, sizeof(size_t));
        offset += sizeof(size_t);
        size_t seg_size;
        memcpy(&seg_size, buf + offset, sizeof(size_t));
        offset += sizeof(size_t);
        levels_offsets = decltype(levels_offsets)(level_size);
        size_t *p = (size_t *)(buf + offset);
        ...
        if (!inner_disk) {
            ...
            for (auto i = 0; i < block_count; i++) {
                read_block(file_handler, buf, i+1);
    ...
    // todo: implement a disk version
    ApproxPos search_disk(int file_handler, const K&key, int *c, int *lc) {
        auto k = std::max(first_key, key);
    ...
    void write_file(int file) {
        ...
        // write offsets info
        size_t *pointer = (size_t *)(buf+sizeof(K));
