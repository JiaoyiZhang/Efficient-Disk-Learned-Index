...
uint64_t read_bit_fix_T(uint8_t* in, int l, int to_find, double slope, double start_key, int start)
// uint32_t read_bit_fix_T(uint8_t* in, int l, int to_find, double slope, double start_key, int start)
{
  ...
  uint64_t out = value + (long long)pred;
...
template <typename T>
T read_bit_fix_int_wo_round(const uint8_t* in, uint8_t l, int to_find, double slope, double start_key, T min_gap)
{
  uint64_t find_bit = to_find * (int)l;
  uint64_t start_byte = find_bit / 8;
  uint8_t start_bit = find_bit % 8;
  ...
  T value = (decode_64 & (((T)1 << (uint8_t)(l - 1)) - 1));
  int64_t out = (start_key + (double)to_find * slope);
  out = std::max((int64_t)0, out);
  if (!sign)
  {
    out = out - value * min_gap;
  }
  else
  {
    out = out + value * min_gap;
