...
namespace Codecset {
    ...
    public:
        ...
        uint8_t* encodeArray8_int(T* in, const size_t length, uint8_t* res, size_t nvalue, T upper_limit) {
            array = in;
            ...
            for (int i = 0;i < segment_total;i++) {
                uint64_t tmp_size = newsegment_size(segment_index[i], segment_index[i + 1] - 1, upper_limit);
                total_byte += tmp_size;
            ...
            while (cost_decline > 0) {
                ...
                // merge(nvalue);
                merge_both_direction(nvalue, length, upper_limit);
            ...
            if(segment_number<=1){
                ...
                newsegment(block_size * nvalue, block_size * nvalue+length - 1, upper_limit);
                // std::cout<<nvalue<<" "<<segment_index[1]-segment_index[0]<<" "<< start_key[0]<<" "<< slope[0]<<std::endl;
            }
            else{
                segment_index.push_back(block_size * nvalue + length);
                for (int i = 0;i < segment_number;i++) {
                    newsegment(segment_index[i], segment_index[i + 1] - 1, upper_limit);
        ...
        void merge_both_direction(int nvalue, int length, T upper_limit) {
            ...
            while (segment_num < total_segments) {
                ...
                if (segment_num == total_segments - 1) {
                    ...
                    uint32_t merge_cost_front = newsegment_size(new_segment_index[last_merged_segment], segment_index[segment_num + 1] - 1, upper_limit);
                ...
                uint32_t merge_cost_back = newsegment_size(segment_index[segment_num], segment_index[segment_num + 2] - 1, upper_limit);
        ...
        uint64_t newsegment_size(uint32_t origin_index, uint32_t end_index, T upper_limit) {
            ...
            for (uint32_t j = origin_index;j <= end_index;j++) {
                // T tmp_val;
                int128_t pred = theta0 + final_slope * (double)(j - origin_index);
                // int64_t pred64 = theta0 + final_slope * (double)(j - origin_index);

                T next_val = upper_limit, range_tmp_err;
                if(j != end_index){
                    next_val = std::max(array[j+1]-1, array[j]);
                }
                if (array[j] > pred)
                {
                    range_tmp_err = array[j] - pred;
                    signvec.emplace_back(true); // means positive
                }
                else if(array[j] <= pred && pred <= next_val){
                    // within the range
                    range_tmp_err = 0;
                    signvec.emplace_back(true); // means positive
                } else
                {
                    range_tmp_err = pred - next_val;
                    signvec.emplace_back(false); // means negative
                }
                ...
                // delta_final.emplace_back(tmp_val);
                delta_final.emplace_back(range_tmp_err);
            ...
            // }
            if (range_max_error) {
                delta_final_max_bit = bits_int_T<T>(range_max_error) + 1;
            }
            // // for debug info
            // uint8_t max_bit = bits_int_T(final_max_error) + 1;
            // uint8_t range_max_bit = (int)(bits_int_T(range_max_error) + 1);
            // if((int)range_max_bit<(int)max_bit){
            //     std::cout<< "\nmax_error: " << final_max_error << std::endl;
            //     std::cout<< "max_bit: " << (int)max_bit << std::endl;
            //     std::cout<< "range_max_error: " << range_max_error << std::endl;
            //     std::cout<< "range max_bit: " << (int)range_max_bit<< std::endl;
            //     std::cout<<"\t\t\t\tsave bits:"<<((int)max_bit-(int)range_max_bit)*length<<std::endl;
            // }
        ...
        void newsegment(uint32_t origin_index, uint32_t end_index, T upper_limit) {
