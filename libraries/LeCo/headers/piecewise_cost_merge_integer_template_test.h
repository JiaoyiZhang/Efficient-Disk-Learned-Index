 ...
 public:
  ...
  int simdSearch(int l, int r, int32_t x, std::vector<uint32_t>& a) {
    ...
    for (int i = l; i < r; i += 8) {
      ...
      if (bitmask > 0) return i + (__builtin_ctz(bitmask) >> 2);
    }
    return -1;
  }
  ...
  uint64_t newsegment_size(uint32_t origin_index, uint32_t end_index) {
    ...
    for (int j = origin_index; j <= end_index; j++) {
      int64_t tmp =
          array[j] -
          (long long)(theta0 + final_slope * (double)(j - origin_index));
  ...
  void newsegment(uint32_t origin_index, uint32_t end_index) {
    ...
    uint8_t* descriptor =
        (uint8_t*)malloc((end_index - origin_index + 1) * sizeof(T) * 4);
    ...
    if (delta_final_max_bit) {
      out = write_delta_int_T(delta_final, signvec, out, delta_final_max_bit,
                              (end_index - origin_index + 1));
    }
  ...
  void newsegment_2(uint32_t origin_index, uint32_t end_index) {
    uint8_t* descriptor =
        (uint8_t*)malloc((end_index - origin_index + 1) * sizeof(T) * 2);
    uint8_t* out = descriptor;
    memcpy(out, &origin_index, sizeof(origin_index));
    out += sizeof(origin_index);
  ...
  uint8_t* encodeArray8_int(T* in, const size_t length, uint8_t* res,
                            size_t nvalue) {
    if (nvalue == 0) {
      array = in;
    }
    ...
    for (int i = nvalue * block_size; i < nvalue * block_size + length - 1;
         i++) {
    ...
    for (int i = 0; i < length - 2; i++) {
      uint32_t delta_tmp_bit =
          cal_bits(delta_first_layer[i], delta_first_layer[i + 1]);
    ...
    for (int aim_bit = min_second_bit; aim_bit <= max_second_bit; aim_bit++) {
      ...
      for (int j = 0; j < length - 2; j++) {
        if (delta_second_bits[j] == aim_bit) {
          // aim_bit is the initiate bit of the two segment, first check whether
          // merging these two segments can reduce the cost if so, scan left &
          // scan right to merge more segments into it
          int segment_id = lower_bound(j, segment_index.size(), segment_index);
          int former_index =
              segment_index[segment_id];  // former_index ~ start_index - 1
          int start_index = segment_index[segment_id + 1];
          int now_index =
              segment_index[segment_id + 2] - 1;  // start_index ~ now_index
          int left_bit_origin = cal_bits(segment_min_delta[segment_id],
                                         segment_max_delta[segment_id]);
          int right_bit_origin = cal_bits(segment_min_delta[segment_id + 1],
                                          segment_max_delta[segment_id + 1]);
          // need to compare to [seg+1]-1

          int64_t new_max_delta = std::max(segment_max_delta[segment_id],
                                           segment_max_delta[segment_id + 1]);
          int64_t new_min_delta = std::min(segment_min_delta[segment_id],
                                           segment_min_delta[segment_id + 1]);
          new_max_delta =
              std::max(new_max_delta,
                       delta_first_layer[segment_index[segment_id + 1] - 1]);
          new_min_delta =
              std::min(new_min_delta,
                       delta_first_layer[segment_index[segment_id + 1] - 1]);
          ...
          if (merged_cost - origin_cost < overhead &&
              segment_id + 1 < segment_index.size()) {
            // merge
          ...
          while (segment_id_search_left >= 0) {
            ...
            int64_t left_max_delta =
                std::max(segment_max_delta[segment_id_search_left],
                         segment_max_delta[segment_id]);
            int64_t left_min_delta =
                std::min(segment_min_delta[segment_id_search_left],
                         segment_min_delta[segment_id]);
            // need to compare to the delta between left segment and the current
            // segment
            left_max_delta =
                std::max(left_max_delta,
                         delta_first_layer[segment_index[segment_id] - 1]);
            left_min_delta =
                std::min(left_min_delta,
                         delta_first_layer[segment_index[segment_id] - 1]);

            int delta_new_bit = cal_bits(left_min_delta, left_max_delta);
            int origin_left_delta_bit =
                cal_bits(segment_min_delta[segment_id_search_left],
                         segment_max_delta[segment_id_search_left]);
            int origin_right_delta_bit = cal_bits(
                segment_min_delta[segment_id], segment_max_delta[segment_id]);

            int origin_cost =
                (segment_index[segment_id] - left_index) *
                    origin_left_delta_bit +
                (segment_index[segment_id + 1] - segment_index[segment_id]) *
                    origin_right_delta_bit;
            int merged_cost =
                delta_new_bit * (segment_index[segment_id + 1] - left_index);

            if (merged_cost - origin_cost < overhead &&
                segment_id < segment_index.size()) {
              // merge
          ...
          while (segment_id_search_right + 1 < segment_index.size()) {
            // std::cout<<"right"<<segment_id_search_right<<" /
            // "<<segment_index_new.size()<<std::endl;
            int right_index = segment_index[segment_id_search_right + 1];
            int64_t right_max_delta =
                std::max(segment_max_delta[segment_id_search_right],
                         segment_max_delta[segment_id]);
            int64_t right_min_delta =
                std::min(segment_min_delta[segment_id_search_right],
                         segment_min_delta[segment_id]);
            // need to compare to the delta between seg & seg+1
            right_max_delta = std::max(
                right_max_delta,
                delta_first_layer[segment_index[segment_id_search_right] - 1]);
            right_min_delta = std::min(
                right_min_delta,
                delta_first_layer[segment_index[segment_id_search_right] - 1]);

            int delta_new_bit = cal_bits(right_min_delta, right_max_delta);
            int origin_left_delta_bit = cal_bits(segment_min_delta[segment_id],
                                                 segment_max_delta[segment_id]);
            int origin_right_delta_bit =
                cal_bits(segment_min_delta[segment_id_search_right],
                         segment_max_delta[segment_id_search_right]);

            int origin_cost =
                (right_index - segment_index[segment_id_search_right]) *
                    origin_right_delta_bit +
                (segment_index[segment_id_search_right] - now_index) *
                    origin_left_delta_bit;
    ...
    for (int i = 0; i < segment_total; i++) {
      uint64_t tmp_size =
          newsegment_size(segment_index[i], segment_index[i + 1] - 1);
      total_byte += tmp_size;
    ...
    while (cost_decline > 0) {
      ...
      double compressrate =
          (total_byte) * 100.0 / (sizeof(T) * block_size * 1.0);
      // std::cout << "try " << iter << " segment number " <<
      // (int)segment_index.size() << " resulting compression rate: " <<
      // std::setprecision(4) << compressrate << std::endl;
      cost_decline = cost_decline - total_byte;
      double cost_decline_percent =
          cost_decline * 100.0 / (sizeof(T) * block_size * 1.0);
    ...
    for (auto item : segment_index) {
      art_build_vec.push_back(
          (KeyValue<uint32_t>){item, segment_index_total_idx});
      // std::cout<<item<<std::endl;
      // auto tmp = btree_total.insert(std::make_pair(item,
      // segment_index_total_idx)); auto tmp = alex_tree.insert(item,
    ...
    if (nvalue == block_num - 1) {
      // auto tmp = btree_total.insert(std::make_pair(block_num * block_size,
      // segment_index_total_idx)); std::cout<<block_num * block_size<<"
      // "<<segment_index_total_idx<<std::endl; auto tmp =
      // alex_tree.insert(block_num * block_size, segment_index_total_idx);
      art_build_vec.push_back((KeyValue<uint32_t>){block_num * block_size,
                                                   segment_index_total_idx});
      alex_build_vec.push_back(
          std::make_pair(block_num * block_size, segment_index_total_idx));
    }
    segment_index.clear();
  ...
  void merge_both_direction(int nvalue, int length) {
    ...
    int start_index =
        segment_index[0];  // before the start_index is the finished blocks
    ...
    while (segment_num < total_segments) {
      // std::cout<<"segment_num: "<<segment_num <<" /
      // "<<total_segments<<std::endl;

      if (segment_num == total_segments - 1) {
        // only can try merging with former one
        int last_merged_segment = new_segment_length.size() - 1;
        uint32_t init_cost_front = segment_length[segment_num] +
                                   new_segment_length[last_merged_segment];
        uint32_t merge_cost_front =
            newsegment_size(new_segment_index[last_merged_segment],
                            segment_index[segment_num + 1] - 1);
        int saved_cost_front = init_cost_front - merge_cost_front;
        if (saved_cost_front > 0) {
          totalbyte_after_merge -=
              new_segment_length[new_segment_length.size() - 1];
      ...
      uint32_t init_cost_front =
          segment_length[segment_num] + new_segment_length[last_merged_segment];
      uint32_t merge_cost_front =
          newsegment_size(new_segment_index[last_merged_segment],
                          segment_index[segment_num + 1] - 1);
      int saved_cost_front = init_cost_front - merge_cost_front;
      ...
      uint32_t init_cost_back =
          segment_length[segment_num] + segment_length[segment_num + 1];
      uint32_t merge_cost_back = newsegment_size(
          segment_index[segment_num], segment_index[segment_num + 2] - 1);
    ...
    for (int i = 0; i < block_start_vec_total.size(); i++) {
      ...
      if (maxerror) {
        if (maxerror >= sizeof(T) * 8 - 1) {
          // read_all_default(tmpin, 0, 0, segment_length, maxerror, theta1,
          // theta0, res);
        } else {
          read_all_bit_fix<T>(tmpin, 0, 0, segment_length, maxerror, theta1,
                              theta0, res);
        }
      } else {
  ...
  int get_segment_id(int to_find) {
    ...
    // int segment_id = lower_bound(to_find, segment_index_total.size(),
    // segment_index_total);
  ...
  T randomdecodeArray8(int segment_id, uint8_t* in, int to_find, uint32_t* out,
                       size_t nvalue) {
    ...
    // int segment_id = binarySearch2(0, length-1, to_find,
    // segment_index_total);
    ...
    if (maxerror) {
      // tmp_val = read_bit_fix_int_float<T>(tmpin, maxerror, to_find-start_ind,
      // theta1, theta0); tmp_val = read_bit_fix_int_float<T>(tmpin, maxerror,
      // to_find - start_ind, theta1, theta0);
      tmp_val = read_bit_fix_T(tmpin, maxerror, to_find - start_ind,
                               (double)theta1, theta0, 0);
    } else {
  ...
  uint64_t summation(uint8_t* in, const size_t l, size_t nvalue) { return 0; }
  ...
  uint32_t* decodeArray(uint32_t* in, const size_t length, uint32_t* out,
                        size_t nvalue) {
    std::cout << "Haven't implement. Please try uint8_t one..." << std::endl;
    return out;
  }
  uint32_t randomdecodeArray(uint32_t* in, const size_t l, uint32_t* out,
                             size_t nvalue) {
    std::cout << "Haven't implement. Please try uint8_t one..." << std::endl;
    return 1;
  }
  uint32_t get_total_byte() { return total_byte_total; }
