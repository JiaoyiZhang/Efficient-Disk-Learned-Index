...
// #endif
    ...
    // std::cout<<"nvalue "<<nvalue<<" number_left "<<number_left<<" nvalue - l
    // "<<nvalue - l<<std::endl; std::cout<<"to_find "<<l<<" block_size
    // "<<block_size<<" bit "<<b<<std::endl;
    ...
    if (32 - bit_left >= b) {
      ...
      recover =
          ((in[1] & (((1U << (b + bit_left - 32)) - 1))) << (32 - bit_left)) +
          (in[0] >> bit_left);
      recover += m;
      return recover;
    }
    ...
    /*
       ...
       //std::cout<<"number_left: "<<number_left<<" number_occupy:
       "<<number_occupy<<" bit_left: "<<bit_left<<std::endl;
       ...
       if(32-bit_left>=b){
           ...
           recover = (( in[1]&(((1U<<(b+bit_left-32))-1)) )<<(32-bit_left)) +
       (in[0]>>bit_left); recover +=m; return recover;
       }
   */
  }
  ...
  uint32_t *decodeArray8(uint8_t *in, const size_t length, uint32_t *out,
                         size_t nvalue) {
    uint32_t *tmpin = reinterpret_cast<uint32_t *>(in);
    return decodeArray(tmpin, length, out, nvalue);
  }
  uint32_t randomdecodeArray8(uint8_t *in, const size_t l, uint32_t *out,
                              size_t nvalue) {
    uint32_t *tmpin = reinterpret_cast<uint32_t *>(in);
  ...
  uint32_t get_block_nums() { return 1; }
  void destroy() {}
  std::string name() const { return "FrameofReference"; }
};
