...
int main(int argc, const char* argv[]) {
  ...
  int blocks = atoi(argv[2]);  // sosd ->
  int delta = atoi(argv[3]);   // sosd (8, 12)
  int model_size = atoi(argv[4]);
  // alternatives : Delta_int, Delta_cost, Delta_cost_merge, FOR_int, Leco_int,
  // Leco_cost, Leco_cost_merge_hc,  Leco_cost_merge, Leco_cost_merge_double
  ...
  for (int i = 0; i < blocks; i++) {
    ...
    uint8_t* descriptor =
        (uint8_t*)malloc(block_length * sizeof(leco_type) * 4 + 1000);
  ...
  double cr_through =
      N * sizeof(leco_type) / ((end_cr - start_cr) * 1000000000);
  ...
  for (int j = 0; j < N; j++) {
    if (data[j] != recover[j]) {
      std::cout << "num: " << j << " true is: " << data[j]
                << " predict is: " << recover[j] << std::endl;
  ...
  codec.alex_tree.bulk_load(codec.alex_build_vec.data(),
                            codec.alex_build_vec.size());
  leco_type mark = 0;
  ...
  for (int i = 0; i < ra_pos.size(); i++) {
    ...
    if (i < ra_pos.size() - 1)
      next_segment_id = codec.get_segment_id(ra_pos[i + 1]);
    leco_type tmpvalue = codec.randomdecodeArray8(
        segment_id, block_start_vec[(int)index / block_size], index, NULL,
        block_size);
    mark += tmpvalue;
    segment_id = next_segment_id;
#ifndef NDEBUG
    if (data[index] != tmpvalue) {
      std::cout << "num: " << index << "true is: " << data[index]
                << " predict is: " << tmpvalue << std::endl;
  ...
  std::cout << method << " " << source_file << " " << blocks << " "
            << compressrate << " " << cr_model << " " << cr_wo_model << " "
            << da_ns << " " << ra_ns << " " << cr_through << std::endl;
